...
use rustred::{
    IbpGenerator, Integral, LinearCombination, ThreeLoopBoundaryConfig, ThreeLoopBoundaryError,
    ThreeLoopBoundaryReducer, equal_mass_three_loop_tetrahedron,
};
...
fn check_paw_goldens_and_symmetries(reducer: &ThreeLoopBoundaryReducer) {
    ...
    assert!(matches!(
        ...
        Err(rustred::TwoLoopPipelineError::OutOfCoverage {
            dots: 6,
            max_dots: 4,
            ..
        })
...
fn check_typed_domain_and_resource_failures(reducer: &ThreeLoopBoundaryReducer) {
    ...
    assert!(matches!(
        ...
        Err(ThreeLoopBoundaryError::TwoLoopTopDot(
            rustred::TwoLoopTopDotError::ResourceLimit {
