...
use rustred::{
    ExactRational, FOUR_LOOP_AFFINE_MAP_OPERATION_BOUND, FourLoopGenuineClassifier,
    FourLoopGenuineConfig, FourLoopGenuineCornerType, FourLoopGenuineWitness,
    FourLoopHaloColumnKey, FourLoopHaloConfig, FourLoopHaloError, FourLoopHaloMapper,
    FourLoopTopology, IbpGenerator, Integral, MassiveVacuumMaster, MasterProduct,
};
...
fn check_all_affine_maps_and_raw_rows() {
    ...
    for kind in FourLoopGenuineCornerType::ALL {
        let classifier = if kind.reference_topology() == rustred::FourLoopTopology::H {
            &h
        } else {
            &x
        };
