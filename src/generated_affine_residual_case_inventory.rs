...
use crate::generated_affine_residual_boolean_cover::{
    GENERATED_AFFINE_RESIDUAL_BOOLEAN_COVER_V1_SCHEMA, GeneratedAffineResidualBooleanAtomPolarity,
    ...
    GeneratedAffineResidualBooleanPointStats,
    GeneratedAffineResidualBooleanPriorActionableSourceView,
    GeneratedAffineResidualBooleanPriorExceptionalPredicateSourceView,
    GeneratedAffineResidualBooleanPriorExceptionalSourceView,
    GeneratedAffineResidualBooleanPriorGuardClassSourceView,
    GeneratedAffineResidualBooleanPriorGuardSourceView,
    GeneratedAffineResidualBooleanPriorUnsupportedSourceView,
    GeneratedAffineResidualBooleanReadyTerminalLimits,
...
use crate::generated_affine_residual_group_exact_publication_handoff::publication_epoch_owner::CommittedExceptionalSingletonSource;
...
pub(crate) struct GeneratedAffineResidualCaseInventoryLimits {
    ...
    pub max_prior_unsupported_terminals: usize,
    pub max_prior_actionable_terminals: usize,
    pub max_prior_exceptional_domain_terminals: usize,
    pub max_prior_exceptional_leak_terminals: usize,
    pub max_cases: usize,
...
impl Default for GeneratedAffineResidualCaseInventoryLimits {
    fn default() -> Self {
        ...
        Self {
            ...
            max_prior_unsupported_terminals: 256_000_000,
            max_prior_actionable_terminals: 256_000_000,
            max_prior_exceptional_domain_terminals: 256_000_000,
            max_prior_exceptional_leak_terminals: 256_000_000,
...
pub(crate) struct GeneratedAffineResidualInventoryAuthenticationStats {
    ...
    boolean_prior_authority_index_comparison_bound: usize,
    boolean_prior_projection_payload_comparison_bound: usize,
...
impl GeneratedAffineResidualInventoryAuthenticationStats {
    inventory_authentication_stats_getters!(
        ...
        boolean_prior_authority_index_comparison_bound,
        boolean_prior_projection_payload_comparison_bound,
...
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
enum GeneratedAffineResidualInventoryActionableSource {
    PriorActionable,
...
enum GeneratedAffineResidualInventoryTerminalBinding {
    ...
    Actionable {
        case_ordinal: usize,
        source: GeneratedAffineResidualInventoryActionableSource,
    },
}
...
enum GeneratedAffineResidualCaseAuthoritySource {
    ...
    CommittedExceptionalSingleton {
        source: Arc<CommittedExceptionalSingletonSource>,
        anchor_offsets: Arc<Vec<Vec<Integer>>>,
        stable_identity: ExactStructuralIdentity,
    },
}
...
impl GeneratedAffineResidualCaseAuthority {
    pub(crate) fn try_new(
        ...
        catch_unwind(AssertUnwindSafe(|| {
            ...
            Ok(Self {
                source: GeneratedAffineResidualCaseAuthoritySource::LegacyInventory(inventory),
                case_ordinal,
                group_ordinal,
                limits,
                stats,
    ...
    /// Admit one event-bound exceptional-domain leaf as a fresh singleton
    /// case/group.  The source can only be minted by consuming an authenticated
    /// publication-epoch lease; this constructor replays its retained parent
    /// generic-source authority and never treats the publication row as an IBP
    /// source.
    pub(crate) fn try_new_committed_exceptional_singleton(
        ...
        catch_unwind(AssertUnwindSafe(|| {
            ...
            // All allocation-sized work is admitted before replaying the
            ...
            let direct_owner_retained_bytes_excluding_source = case_authority_checked_sum(
                "committed exceptional case-authority retained bytes excluding source",
                [
                    size_of::<Self>(),
                    observed_anchor_bytes,
    ...
    /// Bytes owned by this authority allocation itself. The retained source
    /// pointee is deliberately excluded and is accounted by its replayable
    /// owner.
    pub(crate) const fn owner_retained_bytes_excluding_source(&self) -> usize {
        match &self.source {
            GeneratedAffineResidualCaseAuthoritySource::LegacyInventory(_) => size_of::<Self>(),
    ...
    pub(crate) fn same_inventory_allocation(
        ...
        matches!(
            &self.source,
            GeneratedAffineResidualCaseAuthoritySource::LegacyInventory(retained)
    ...
    pub(crate) fn same_inventory_allocation_as(&self, other: &Self) -> bool {
        matches!(
            ...
            (
                GeneratedAffineResidualCaseAuthoritySource::LegacyInventory(left),
                GeneratedAffineResidualCaseAuthoritySource::LegacyInventory(right)
        ...
        catch_unwind(AssertUnwindSafe(|| {
            let scope_comparison_bytes = match &self.source {
                ...
                GeneratedAffineResidualCaseAuthoritySource::CommittedExceptionalSingleton {
                    ...
                    return source.authenticated_source_row_view(
                        family,
                        context,
                        source_row_ordinal,
                        limits,
...
impl fmt::Debug for GeneratedAffineResidualInventoryTerminalSourceView<'_> {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        let kind = match self {
            ...
            Self::PriorUnsupported(_) => "PriorUnsupported",
            Self::PriorActionable(_) => "PriorActionable",
            Self::PriorExceptionalDomain(_) => "PriorExceptionalDomain",
            Self::PriorExceptionalLeak(_) => "PriorExceptionalLeak",
        };
        formatter
...
impl<'source> GeneratedAffineResidualInventoryCaseSourceView<'source> {
    pub(crate) const fn affine_map(self) -> &'source ResidualAffineIntegerMap {
        match self {
            ...
            Self::PriorActionable(view) => view.affine_map(),
            Self::PriorExceptionalDomain(view) | Self::PriorExceptionalLeak(view) => {
                view.affine_map()
            }
        }
    }
    ...
    pub(crate) const fn guard_count(self) -> usize {
        match self {
            ...
            Self::PriorActionable(view) => view.guard_entry_count(),
            Self::PriorExceptionalDomain(view) | Self::PriorExceptionalLeak(view) => {
                view.guard_entry_count()
            }
        }
    }
    ...
    pub(crate) const fn exceptional_predicate_count(self) -> usize {
        match self {
            Self::PriorExceptionalDomain(view) | Self::PriorExceptionalLeak(view) => {
                view.predicate_count()
            }
            Self::Initial(_) | Self::PriorActionable(_) => 0,
        }
    }
}
...
pub(crate) enum GeneratedAffineResidualCaseGeometryView<'source> {
    ...
    CommittedExceptional(&'source CommittedExceptionalSingletonSource),
}
...
impl<'source> GeneratedAffineResidualCaseGeometryView<'source> {
    pub(crate) fn ambient_arity(self) -> usize {
        match self {
            ...
            Self::CommittedExceptional(source) => source.ambient_arity(),
        }
    }
    ...
    pub(crate) fn free_positions(self) -> &'source [usize] {
        match self {
            ...
            Self::CommittedExceptional(source) => source.free_positions(),
        }
    }
    ...
    pub(crate) fn constant(self, row: usize) -> Option<&'source Integer> {
        match self {
            ...
            Self::CommittedExceptional(source) => source.constants().get(row),
        }
    }
    ...
    pub(crate) fn compact_linear_coefficient(
        ...
        match self {
            ...
            Self::CommittedExceptional(source) => {
                if row >= source.ambient_arity() || free_ordinal >= source.free_positions().len() {
                    return None;
                }
                let compact_position = row
                    .checked_mul(source.free_positions().len())?
                    .checked_add(free_ordinal)?;
                source.compact_affine_matrix().get(compact_position)
            }
        }
    }
}
...
impl<'source> GeneratedAffineResidualCaseSourceView<'source> {
    pub(crate) fn geometry(self) -> GeneratedAffineResidualCaseGeometryView<'source> {
        match self.inner {
            ...
            GeneratedAffineResidualCaseSourceViewInner::CommittedExceptional(source) => {
                GeneratedAffineResidualCaseGeometryView::CommittedExceptional(source)
            }
        }
    }
    ...
    pub(crate) fn exceptional_predicate(
        ...
        let prior = match self.inner {
            GeneratedAffineResidualCaseSourceViewInner::Legacy(
        ...
        let predicate = prior.predicate(predicate_ordinal)?;
        Some(GeneratedAffineResidualCaseExceptionalPredicateSourceView {
            predicate_ordinal,
            locus_ordinal: predicate.locus_ordinal(),
            kind: predicate.kind(),
            polynomial: predicate.polynomial(),
...
fn neutral_legacy_guard_entry<'source>(
    ...
    match source {
        ...
        GeneratedAffineResidualInventoryCaseSourceView::PriorActionable(prior) => {
            neutral_prior_guard_entry(prior.guard_entry(entry_ordinal)?, entry_ordinal)
        }
        GeneratedAffineResidualInventoryCaseSourceView::PriorExceptionalDomain(prior)
        | GeneratedAffineResidualInventoryCaseSourceView::PriorExceptionalLeak(prior) => {
            neutral_prior_guard_entry(prior.guard_entry(entry_ordinal)?, entry_ordinal)
...
fn neutral_prior_guard_entry<'source>(
    entry: GeneratedAffineResidualBooleanPriorGuardSourceView<'source>,
    entry_ordinal: usize,
) -> Option<GeneratedAffineResidualCaseGuardSourceView<'source>> {
    let class = match entry.class() {
        ...
        GeneratedAffineResidualBooleanPriorGuardClassSourceView::BaseAssumption {
            condition_polynomial,
        } => GeneratedAffineResidualCaseGuardClassSourceView::BaseAssumption(condition_polynomial),
...
impl GeneratedAffineResidualCaseInventoryCertificate {
    ...
    > {
        ...
        let source = match (record.binding, record.outcome, source_record.source()) {
            ...
            (
                GeneratedAffineResidualInventoryTerminalBinding::PriorUnsupported,
                GeneratedAffineResidualInventoryTerminalOutcome::AffineUnsupported,
                GeneratedAffineResidualBooleanTerminalSourceView::PriorUnsupported(source),
            ) if source_record.outcome()
                ...
                preflight_inventory_terminal_payload_references(
                    ...
                    checked_add(
                        ...
                        checked_add(
                            "terminal payload references",
                            source
                                .atom_count(GeneratedAffineResidualBooleanAtomPolarity::EqualZero),
                            source.atom_count(GeneratedAffineResidualBooleanAtomPolarity::NonZero),
                        )?,
                        source.unsupported_reason_count(),
                    )?,
                )?;
                authenticate_prior_unsupported_source_view(source)?;
                GeneratedAffineResidualInventoryTerminalSourceView::PriorUnsupported(source)
            }
            (
                GeneratedAffineResidualInventoryTerminalBinding::Actionable {
                    case_ordinal,
                    source: GeneratedAffineResidualInventoryActionableSource::PriorActionable,
                },
                GeneratedAffineResidualInventoryTerminalOutcome::Actionable,
                GeneratedAffineResidualBooleanTerminalSourceView::PriorActionable(source),
            ) if source_record.outcome()
                == GeneratedAffineResidualBooleanTerminalOutcome::PriorActionable =>
            {
                ...
                preflight_inventory_terminal_payload_references(
                    ...
                    checked_add(
                        ...
                        checked_add(
                            "terminal payload references",
                            source.constant_count(),
                            source.free_position_count(),
                        )?,
                        source.guard_entry_count(),
                    )?,
                )?;
                authenticate_prior_actionable_source_view(source)?;
                GeneratedAffineResidualInventoryTerminalSourceView::PriorActionable(source)
            }
            (
                GeneratedAffineResidualInventoryTerminalBinding::Actionable {
                    case_ordinal,
                    source: GeneratedAffineResidualInventoryActionableSource::PriorExceptionalDomain,
                },
                GeneratedAffineResidualInventoryTerminalOutcome::Actionable,
                GeneratedAffineResidualBooleanTerminalSourceView::PriorExceptionalDomain(source),
            ) if source_record.outcome()
                == GeneratedAffineResidualBooleanTerminalOutcome::PriorExceptionalDomain =>
            {
                ...
                preflight_inventory_terminal_payload_references(
                    stats,
                    limits,
                    inventory_exceptional_payload_reference_count(source)?,
                )?;
                authenticate_prior_exceptional_source_view(source)?;
                GeneratedAffineResidualInventoryTerminalSourceView::PriorExceptionalDomain(source)
            }
            (
                GeneratedAffineResidualInventoryTerminalBinding::Actionable {
                    case_ordinal,
                    source: GeneratedAffineResidualInventoryActionableSource::PriorExceptionalLeak,
                },
                GeneratedAffineResidualInventoryTerminalOutcome::Actionable,
                GeneratedAffineResidualBooleanTerminalSourceView::PriorExceptionalLeak(source),
            ) if source_record.outcome()
                == GeneratedAffineResidualBooleanTerminalOutcome::PriorExceptionalLeak =>
            {
                ...
                authenticate_prior_exceptional_source_view(source)?;
                GeneratedAffineResidualInventoryTerminalSourceView::PriorExceptionalLeak(source)
            }
            (
    ...
    > {
        ...
        let source = match terminal.source() {
            ...
            GeneratedAffineResidualInventoryTerminalSourceView::PriorActionable(source) => {
                GeneratedAffineResidualInventoryCaseSourceView::PriorActionable(source)
            }
            GeneratedAffineResidualInventoryTerminalSourceView::PriorExceptionalDomain(source) => {
                GeneratedAffineResidualInventoryCaseSourceView::PriorExceptionalDomain(source)
            }
            GeneratedAffineResidualInventoryTerminalSourceView::PriorExceptionalLeak(source) => {
                GeneratedAffineResidualInventoryCaseSourceView::PriorExceptionalLeak(source)
            }
            _ => return Err(GeneratedAffineResidualCaseInventoryError::OutcomeInvariant),
        };
        let group = self
...
fn accumulate_inventory_boolean_navigation(
    ...
    stats.boolean_prior_authority_index_comparison_bound = checked_add(
        "Boolean prior authority index comparison bound",
        stats.boolean_prior_authority_index_comparison_bound,
        additional.prior_authority_index_comparison_bound(),
    )?;
    stats.boolean_prior_projection_payload_comparison_bound = checked_add(
        "Boolean prior projection payload comparison bound",
        stats.boolean_prior_projection_payload_comparison_bound,
        additional.prior_projection_payload_comparison_bound(),
    )?;
    Ok(())
}
...
fn inventory_exceptional_payload_reference_count(
    source: GeneratedAffineResidualBooleanPriorExceptionalSourceView<'_>,
) -> Result<usize, GeneratedAffineResidualCaseInventoryError> {
    [
        source.constant_count(),
        source.free_position_count(),
    ...
    .try_fold(0usize, |total, value| {
        checked_add("terminal payload references", total, value)
    })
}
...
fn validate_prior_guards<'source>(
    guard_count: usize,
    ...
    for position in 0..guard_count {
        let entry =
            guard_at(position).ok_or(GeneratedAffineResidualCaseInventoryError::SourceBinding)?;
        let _ = entry.structural_locus_ordinal();
        let _ = entry.mapped_polynomial();
        let _ = entry.composition_stats();
        match entry.class() {
...
fn validate_prior_map_positions<'source>(
    map: &ResidualAffineIntegerMap,
    constant_count: usize,
    mut constant_at: impl FnMut(usize) -> Option<&'source Integer>,
    free_position_count: usize,
    mut free_position_at: impl FnMut(usize) -> Option<usize>,
    _stats: &mut GeneratedAffineResidualCaseInventoryStats,
    _limits: GeneratedAffineResidualCaseInventoryLimits,
) -> Result<(), GeneratedAffineResidualCaseInventoryError> {
    if constant_count != map.ambient_arity() || free_position_count != map.free_positions().len() {
        return Err(GeneratedAffineResidualCaseInventoryError::SourceBinding);
    }
    for position in 0..constant_count {
        let observed = constant_at(position)
            .ok_or(GeneratedAffineResidualCaseInventoryError::SourceBinding)?;
        let expected = map
            .constant(position)
            .ok_or(GeneratedAffineResidualCaseInventoryError::SourceBinding)?;
    ...
    for position in 0..free_position_count {
        if free_position_at(position) != map.free_positions().get(position).copied() {
            return Err(GeneratedAffineResidualCaseInventoryError::SourceBinding);
        }
    }
    Ok(())
...
fn inspect_prior_actionable_geometry(
    view: GeneratedAffineResidualBooleanPriorActionableSourceView<'_>,
    builder_scratch_temporary_bytes: usize,
    compilation_overlap_base: usize,
    stats: &mut GeneratedAffineResidualCaseInventoryStats,
    limits: GeneratedAffineResidualCaseInventoryLimits,
    ...
    charge_source_reference_shape(
        ...
        [
            ...
            (view.constant_count(), size_of::<&Integer>()),
            (view.free_position_count(), size_of::<usize>()),
            (
                view.guard_entry_count(),
                size_of::<GeneratedAffineResidualBooleanPriorGuardSourceView<'_>>(),
            ),
        ],
        limits,
    )?;
    let map = view.affine_map();
    validate_prior_map_positions(
        map,
        view.constant_count(),
        |position| view.constant(position),
        view.free_position_count(),
        |position| view.free_position(position),
        stats,
        limits,
    )?;
    validate_prior_guards(
        view.guard_entry_count(),
        |position| view.guard_entry(position),
        stats,
        limits,
    )?;
    inspect_geometry(
...
fn inspect_prior_exceptional_geometry(
    view: GeneratedAffineResidualBooleanPriorExceptionalSourceView<'_>,
    builder_scratch_temporary_bytes: usize,
    compilation_overlap_base: usize,
    stats: &mut GeneratedAffineResidualCaseInventoryStats,
    limits: GeneratedAffineResidualCaseInventoryLimits,
    ...
    charge_source_reference_shape(
        ...
        [
            ...
            (
                view.predicate_count(),
                size_of::<GeneratedAffineResidualBooleanPriorExceptionalPredicateSourceView<'_>>(),
            ),
        ],
        limits,
    )?;
    ...
    let predicate_count = view.predicate_count();
    for position in 0..predicate_count {
        let predicate = view
            .predicate(position)
            .ok_or(GeneratedAffineResidualCaseInventoryError::SourceBinding)?;
        let _ = predicate.locus_ordinal();
        let _ = predicate.kind();
        let _ = predicate.polynomial();
    }
    inspect_geometry(
        map,
        builder_scratch_temporary_bytes,
...
fn validate_prior_unsupported(
    view: GeneratedAffineResidualBooleanPriorUnsupportedSourceView<'_>,
    stats: &mut GeneratedAffineResidualCaseInventoryStats,
    limits: GeneratedAffineResidualCaseInventoryLimits,
) -> Result<(), GeneratedAffineResidualCaseInventoryError> {
    let equal_zero_count = view.atom_count(GeneratedAffineResidualBooleanAtomPolarity::EqualZero);
    let nonzero_count = view.atom_count(GeneratedAffineResidualBooleanAtomPolarity::NonZero);
    let reason_count = view.unsupported_reason_count();
    charge_source_reference_shape(
        stats,
        [
            (
                checked_add(
                    "prior unsupported atom references",
                    equal_zero_count,
                    nonzero_count,
                )?,
                size_of::<crate::generated_affine_residual_boolean_cover::GeneratedAffineResidualBooleanPriorAtomSourceView<'_>>(),
            ),
            (
                reason_count,
    ...
    for polarity in [
        ...
        let count = view.atom_count(polarity);
        for position in 0..count {
            let atom = view
    ...
    for position in 0..reason_count {
...
fn authenticate_prior_unsupported_source_view(
    view: GeneratedAffineResidualBooleanPriorUnsupportedSourceView<'_>,
) -> Result<(), GeneratedAffineResidualCaseInventoryError> {
    for polarity in [
        GeneratedAffineResidualBooleanAtomPolarity::EqualZero,
        ...
        for position in 0..view.atom_count(polarity) {
            let atom = view
                .atom(polarity, position)
                .ok_or(GeneratedAffineResidualCaseInventoryError::SourceBinding)?;
            let _ = atom.locus_ordinal();
    ...
    for position in 0..view.unsupported_reason_count() {
        view.unsupported_reason(position)
            .ok_or(GeneratedAffineResidualCaseInventoryError::SourceBinding)?;
    }
    Ok(())
...
fn authenticate_prior_actionable_source_view(
    view: GeneratedAffineResidualBooleanPriorActionableSourceView<'_>,
) -> Result<(), GeneratedAffineResidualCaseInventoryError> {
    let map = view.affine_map();
    if view.constant_count() != map.ambient_arity()
        || view.free_position_count() != map.free_positions().len()
    {
        return Err(GeneratedAffineResidualCaseInventoryError::SourceBinding);
    }
    for position in 0..view.constant_count() {
        let observed = view
            .constant(position)
            .ok_or(GeneratedAffineResidualCaseInventoryError::SourceBinding)?;
        let expected = map
    ...
    for position in 0..view.free_position_count() {
        if view.free_position(position) != map.free_positions().get(position).copied() {
            return Err(GeneratedAffineResidualCaseInventoryError::SourceBinding);
        }
    }
    for position in 0..view.guard_entry_count() {
        let guard = view
            .guard_entry(position)
            .ok_or(GeneratedAffineResidualCaseInventoryError::SourceBinding)?;
        let _ = guard.structural_locus_ordinal();
        let _ = guard.mapped_polynomial();
        let _ = guard.composition_stats();
        let _ = guard.class().condition_polynomial();
    }
    Ok(())
}

fn authenticate_prior_exceptional_source_view(
    view: GeneratedAffineResidualBooleanPriorExceptionalSourceView<'_>,
) -> Result<(), GeneratedAffineResidualCaseInventoryError> {
    let map = view.affine_map();
    if view.constant_count() != map.ambient_arity()
    ...
    for position in 0..view.predicate_count() {
        let predicate = view
            .predicate(position)
            .ok_or(GeneratedAffineResidualCaseInventoryError::SourceBinding)?;
        let _ = predicate.locus_ordinal();
...
impl GeneratedAffineResidualInventoryRawOwnedCensus {
    fn authenticates_stats(self, stats: GeneratedAffineResidualCaseInventoryStats) -> bool {
        self.terminals == stats.terminals
            ...
            && self.prior_unsupported_terminals == stats.prior_unsupported_terminals
            && self.prior_actionable_terminals == stats.prior_actionable_terminals
            && self.prior_exceptional_domain_terminals == stats.prior_exceptional_domain_terminals
            && self.prior_exceptional_leak_terminals == stats.prior_exceptional_leak_terminals
...
fn validate_complete_stats_against_limits(
    ...
    for (resource, requested, limit) in [
        ...
        (
            ...
            stats.prior_unsupported_terminals,
            limits.max_prior_unsupported_terminals,
        ),
        (
            "prior actionable terminals",
            stats.prior_actionable_terminals,
            limits.max_prior_actionable_terminals,
        ),
        (
            "prior exceptional-domain terminals",
            stats.prior_exceptional_domain_terminals,
            limits.max_prior_exceptional_domain_terminals,
        ),
        (
            "prior exceptional-leak terminals",
            stats.prior_exceptional_leak_terminals,
            limits.max_prior_exceptional_leak_terminals,
        ),
    ...
    for (ordinal, record) in certificate.terminals.iter().enumerate() {
        ...
        match record.binding {
            ...
            GeneratedAffineResidualInventoryTerminalBinding::PriorUnsupported => {
                ...
                census.prior_unsupported_terminals = bounded_add(
                    "raw prior unsupported terminals",
                    census.prior_unsupported_terminals,
                    1,
                    limits.max_prior_unsupported_terminals,
                )?;
            }
            GeneratedAffineResidualInventoryTerminalBinding::Actionable { source, .. } => {
                if record.outcome != GeneratedAffineResidualInventoryTerminalOutcome::Actionable {
                    return Err(GeneratedAffineResidualCaseInventoryError::ReplayMismatch);
                }
                let (field, limit, resource) = match source {
                    GeneratedAffineResidualInventoryActionableSource::PriorActionable => (
                        &mut census.prior_actionable_terminals,
                        limits.max_prior_actionable_terminals,
                        "raw prior actionable terminals",
                    ),
                    GeneratedAffineResidualInventoryActionableSource::PriorExceptionalDomain => (
                        &mut census.prior_exceptional_domain_terminals,
                        limits.max_prior_exceptional_domain_terminals,
                        "raw prior exceptional-domain terminals",
                    ),
                    GeneratedAffineResidualInventoryActionableSource::PriorExceptionalLeak => (
                        &mut census.prior_exceptional_leak_terminals,
                        limits.max_prior_exceptional_leak_terminals,
                        "raw prior exceptional-leak terminals",
                    ),
                };
                *field = bounded_add(resource, *field, 1, limit)?;
            }
...
> {
    ...
    check_limit(
        "prior unsupported terminals",
        source_stats.prior_unsupported_terminals(),
        limits.max_prior_unsupported_terminals,
    )?;
    check_limit(
        "prior actionable terminals",
        source_stats.prior_actionable_terminals(),
        limits.max_prior_actionable_terminals,
    )?;
    check_limit(
        "prior exceptional-domain terminals",
        source_stats.prior_exceptional_domain_terminals(),
        limits.max_prior_exceptional_domain_terminals,
    )?;
    check_limit(
        "prior exceptional-leak terminals",
        source_stats.prior_exceptional_leak_terminals(),
        limits.max_prior_exceptional_leak_terminals,
    )?;
            ...
            GeneratedAffineResidualBooleanReplayedTerminal::Passthrough(view) => {
                ...
                match (view.outcome(), view.source()) {
                    ...
                    (
                        ...
                        validate_prior_unsupported(source, &mut stats, limits)?;
                        stats.prior_unsupported_terminals = bounded_add(
                            "prior unsupported terminals",
                            stats.prior_unsupported_terminals,
                            1,
                            limits.max_prior_unsupported_terminals,
                    ...
                    (
                        ...
                        let geometry = inspect_prior_actionable_geometry(
                            source,
                        ...
                        stats.prior_actionable_terminals = bounded_add(
                            "prior actionable terminals",
                            stats.prior_actionable_terminals,
                            1,
                            limits.max_prior_actionable_terminals,
                    ...
                    (
                        ...
                        let geometry = inspect_prior_exceptional_geometry(
                            source,
                            record_builder_scratch,
                            record_prefix,
                            &mut stats,
                        ...
                        stats.prior_exceptional_domain_terminals = bounded_add(
                            "prior exceptional-domain terminals",
                            stats.prior_exceptional_domain_terminals,
                            1,
                            limits.max_prior_exceptional_domain_terminals,
                    ...
                    (
                        ...
                        stats.prior_exceptional_leak_terminals = bounded_add(
                            "prior exceptional-leak terminals",
                            stats.prior_exceptional_leak_terminals,
                            1,
                            limits.max_prior_exceptional_leak_terminals,
    ...
    if stats.terminals != terminal_count
        ...
        || stats.prior_unsupported_terminals != source_stats.prior_unsupported_terminals()
        || stats.prior_actionable_terminals != source_stats.prior_actionable_terminals()
        || stats.prior_exceptional_domain_terminals
            != source_stats.prior_exceptional_domain_terminals()
        || stats.prior_exceptional_leak_terminals != source_stats.prior_exceptional_leak_terminals()
        || stats.actionable_terminals != stats.cases
    {
        return Err(GeneratedAffineResidualCaseInventoryError::ConservationInvariant);
    }
    let prior_actionable_total = [
        stats.prior_actionable_terminals,
        stats.prior_exceptional_domain_terminals,
        stats.prior_exceptional_leak_terminals,
    ]
    .into_iter()
    .try_fold(0usize, |total, count| {
        checked_add("prior actionable terminal partition", total, count)
    })?;
    let initial_unsupported = stats
        .affine_unsupported_terminals
        .checked_sub(stats.prior_unsupported_terminals)
        .ok_or(GeneratedAffineResidualCaseInventoryError::ConservationInvariant)?;
    let initial_actionable = stats
        .actionable_terminals
        .checked_sub(prior_actionable_total)
        .ok_or(GeneratedAffineResidualCaseInventoryError::ConservationInvariant)?;
...
mod tests {
    ...
    use crate::generated_affine_residual_case_premises::{
        GeneratedAffineResidualCasePremisesError, GeneratedAffineResidualCasePremisesLimits,
    ...
    use crate::generated_affine_residual_source_authority::{
        ...
        GeneratedAffineResidualSourceAuthorityKind, GeneratedAffineResidualSourcePointLimits,
        GeneratedAffineResidualSourcePointStats,
    };
    ...
    use crate::{
        ...
        GeneratedSectorLiveLeafQueueLimits, GeneratedWhenBadCompilation, GeneratedWhenBadCompiler,
        GeneratedWhenBadLimits, IntegralOrderingPolicy, ParametricArithmeticLimits,
        ParametricElimination, ParametricEliminationLimits, ParametricEliminationOrdering,
        ParametricIbpGenerator, ParametricReductionRuleCandidate, ParametricRuleLimits, SectorMask,
    };
    ...
    fn direct_sunset_terminals(
        name: &str,
    ) -> (
        IntegralFamily,
        ParametricCoefficientContext,
        Vec<Arc<ParametricSectorFormulaAffineTerminalCertificate>>,
    ) {
        ...
        (family, context, terminals)
    }

    fn direct_unsupported_terminal() -> (
        IntegralFamily,
        ParametricCoefficientContext,
        Arc<ParametricSectorFormulaAffineTerminalCertificate>,
    ) {
        let family = massive_tadpole_family("direct-case-authority-unsupported-one-loop");
    ...
    #[test]
    fn direct_formula_singleton_preserves_exact_terminal_geometry_guards_and_rows() {
        let (family, context, mut terminals) =
            direct_sunset_terminals("direct-case-authority-source-views");
        ...
        let terminal_path = Arc::clone(terminal.path_arc());
        let source_owner = Arc::clone(terminal_path.source_arc());
        let weak_terminal = Arc::downgrade(&terminal);
        let weak_path = Arc::downgrade(&terminal_path);
        let weak_source = Arc::downgrade(&source_owner);
        let source_row_count = source_owner.row_span().rows().len();
        assert!(source_row_count > 0);
        let (selected_source_row_ordinal, selected_source_row) = source_owner
            .row_span()
            .rows()
            .iter()
            .enumerate()
            ...
            .unwrap();
        let selected_relation_terms = selected_source_row.terms().len();
        let selected_guard_conditions = selected_source_row.guarded_nonzero_conditions().len();
        assert!(selected_relation_terms > 0);
        assert_eq!(selected_guard_conditions, 0);
        let selected_source_row = selected_source_row as *const ParametricRelation;
        ...
        assert_eq!(authority.case_ordinal(), 0);
        assert_eq!(authority.group_ordinal(), 0);
        assert_eq!(authority.arity(), 3);
        assert_eq!(authority.source_row_count(), source_row_count);
        assert!(authority.same_direct_formula_terminal_allocation(&terminal));
        ...
        assert_eq!(authority.stats().direct_terminal_authentications(), 1);
        assert_eq!(authority.stats().direct_case_authentications(), 1);
        assert_eq!(authority.stats().direct_group_authentications(), 1);
        assert_eq!(authority.stats().direct_guard_scans(), 3);
        assert_eq!(authority.stats().direct_anchor_offset_entries(), 3);
        assert_eq!(authority.stats().direct_anchor_offset_integer_bits(), 0);
        assert_eq!(
        ...
        assert!(matches!(
            authority.authenticated_case_view(&context),
            Err(GeneratedAffineResidualCaseAuthorityError::SourceBinding)
        ));
        assert!(matches!(
        ...
        let case = authority
            ...
            .unwrap();
        assert_eq!(case.ordinal(), 0);
        assert_eq!(case.group_ordinal(), 0);
        assert_eq!(case.ordinal_within_group(), 0);
        let GeneratedAffineResidualCaseSourceLocator::DirectFormula {
            ...
            panic!("direct singleton must expose a typed direct locator")
        };
        assert_eq!(normalized_source_schema, source_owner.schema());
        assert_eq!(path_schema, terminal_path.schema());
        assert_eq!(branch_order_schema, terminal_path.branch_order_schema());
        assert_eq!(request, ParametricSectorFormulaResidualRequest::AnyResidual);
        assert_eq!(yield_ordinal, 1);
        assert_eq!(
            terminal_kind,
            ParametricSectorFormulaResidualKind::Uncovered
        );
        ...
        assert_eq!(
            (0..3)
                .map(|row| geometry.constant(row).unwrap().clone())
                .collect::<Vec<_>>(),
            [
        ...
        assert_eq!(
            geometry.compact_linear_coefficient(0, 0),
            Some(&Integer::from(0))
        );
        assert_eq!(
            geometry.compact_linear_coefficient(1, 0),
            Some(&Integer::from(1))
        );
        assert_eq!(
            geometry.compact_linear_coefficient(2, 0),
            Some(&Integer::from(0))
        );
        assert!(geometry.compact_linear_coefficient(3, 0).is_none());
        assert!(geometry.compact_linear_coefficient(0, 1).is_none());

        let source = case.source();
        assert_eq!(source.guard_count(), 3);
        for (entry_ordinal, (decision_ordinal, structural_locus_ordinal)) in
            [(0, 0), (2, 3), (3, 4)].into_iter().enumerate()
        {
            let guard = source.guard_entry(entry_ordinal).unwrap();
            assert_eq!(guard.entry_ordinal(), entry_ordinal);
            assert_eq!(guard.structural_locus_ordinal(), structural_locus_ordinal);
            assert!(matches!(
                guard.class(),
                GeneratedAffineResidualCaseGuardClassSourceView::BaseAssumption(_)
            ));
            assert!(std::ptr::eq(
                guard.mapped_polynomial(),
                terminal.guards()[entry_ordinal].mapped_polynomial()
            ));
            assert_eq!(
                terminal.guards()[entry_ordinal].decision_ordinal(),
                decision_ordinal
            );
        }
        assert_eq!(source.exceptional_predicate_count(), 0);
        assert!(source.exceptional_predicate(0).is_none());

        let group = authority
            .authenticated_source_neutral_group_view(&context)
            .unwrap();
        assert_eq!(group.ordinal(), 0);
        assert_eq!(group.ambient_arity(), 3);
        assert_eq!(group.case_ordinals(), &[0]);
        assert_eq!(group.anchor_case_ordinal(), 0);
        assert_eq!(group.free_positions(), &[1]);
        assert_eq!(group.anchor_offsets().len(), 1);
        assert_eq!(group.anchor_offsets()[0].len(), 3);
        assert!(group.anchor_offsets()[0].iter().all(Integer::is_zero));

        drop(source_owner);
        drop(terminal_path);
        drop(terminal);
        assert_eq!(weak_terminal.strong_count(), 1);
        assert_eq!(weak_path.strong_count(), 1);
        assert_eq!(weak_source.strong_count(), 1);
        let mut exact_rows = GeneratedAffineResidualCaseSourceRowLimits::default();
        exact_rows.max_source_rows = source_row_count;
        exact_rows.max_relation_terms = selected_relation_terms;
        exact_rows.max_guard_conditions = selected_guard_conditions;
        let selected = authority
            ...
            .unwrap();
        assert_eq!(selected.source_row_ordinal(), selected_source_row_ordinal);
        assert_eq!(selected.stats().source_rows(), source_row_count);
        assert_eq!(selected.stats().relation_terms(), selected_relation_terms);
        assert_eq!(
            selected.stats().guard_conditions(),
            selected_guard_conditions
        );
        assert!(std::ptr::eq(selected.relation(), selected_source_row));
        assert!(matches!(
            authority.authenticated_source_row_view(
                &family,
                &context,
                source_row_count,
                exact_rows,
            ),
            Err(GeneratedAffineResidualCaseAuthorityError::SourceRowOutOfRange)
        ));
        let mut below_rows = exact_rows;
        below_rows.max_source_rows = source_row_count - 1;
        assert!(matches!(
            authority.authenticated_source_row_view(
                &family,
                &context,
            ...
            Err(GeneratedAffineResidualCaseAuthorityError::ResourceLimit {
                ...
                requested,
                limit,
            }) if requested == source_row_count && limit + 1 == source_row_count
        ));
        let mut below_terms = exact_rows;
        below_terms.max_relation_terms = selected_relation_terms - 1;
        assert!(matches!(
            authority.authenticated_source_row_view(
                &family,
                &context,
            ...
            Err(GeneratedAffineResidualCaseAuthorityError::ResourceLimit {
                resource: "source relation terms",
                requested,
                limit,
    ...
    #[test]
    fn direct_formula_singleton_rejects_non_actionable_outcomes_before_replay_limits() {
        let (family, context, terminals) =
            direct_sunset_terminals("direct-case-authority-proved-empty-precedence");
        assert!(matches!(
            terminals[1].outcome(),
            ParametricSectorFormulaAffineTerminalOutcome::ProvedEmpty(_)
        ));
        let mut no_replay = GeneratedAffineResidualCaseAuthorityLimits::default();
        no_replay.max_direct_terminal_replays = 0;
        assert!(matches!(
            GeneratedAffineResidualCaseAuthority::try_new_direct_formula_singleton(
                &family,
                &context,
                Arc::clone(&terminals[1]),
                no_replay,
            ),
            Err(GeneratedAffineResidualCaseAuthorityError::DirectTerminalNotActionable)
        ));
        assert!(matches!(
            GeneratedAffineResidualCaseAuthority::try_new_direct_formula_singleton(
                &family,
                &context,
                Arc::clone(&terminals[0]),
                no_replay,
            ),
            Err(GeneratedAffineResidualCaseAuthorityError::ResourceLimit {
                resource: "direct terminal replays",
                requested: 1,
                limit: 0,
            })
        ));

        let (unsupported_family, unsupported_context, unsupported) = direct_unsupported_terminal();
        assert!(matches!(
            GeneratedAffineResidualCaseAuthority::try_new_direct_formula_singleton(
                &unsupported_family,
                &unsupported_context,
    ...
    #[test]
    fn direct_formula_singleton_anchor_limits_are_exact_and_replayed() {
        let (family, context, terminals) =
            direct_sunset_terminals("direct-case-authority-anchor-limits");
        let terminal = Arc::clone(&terminals[5]);
        let authority = GeneratedAffineResidualCaseAuthority::try_new_direct_formula_singleton(
            &family,
            &context,
            Arc::clone(&terminal),
            GeneratedAffineResidualCaseAuthorityLimits::default(),
        ...
        .unwrap();
        let observed_entries = authority.stats().direct_anchor_offset_entries();
        let observed_bytes = authority.stats().direct_anchor_offset_bytes();
        assert_eq!(observed_entries, 3);
        assert!(observed_bytes > 0);

        let mut exact = GeneratedAffineResidualCaseAuthorityLimits::default();
        exact.max_direct_anchor_offset_entries = observed_entries;
        exact.max_direct_anchor_offset_bytes = observed_bytes;
        GeneratedAffineResidualCaseAuthority::try_new_direct_formula_singleton(
            &family,
            &context,
            Arc::clone(&terminal),
        ...
        let mut below_entries = exact;
        below_entries.max_direct_anchor_offset_entries = observed_entries - 1;
        assert!(matches!(
            GeneratedAffineResidualCaseAuthority::try_new_direct_formula_singleton(
                &family,
                &context,
            ...
            Err(GeneratedAffineResidualCaseAuthorityError::ResourceLimit {
                resource: "direct singleton anchor-offset entries",
                requested,
                limit,
            }) if requested == observed_entries && limit + 1 == observed_entries
        ));

        let mut below_bytes = exact;
        below_bytes.max_direct_anchor_offset_bytes = observed_bytes - 1;
        assert!(matches!(
            GeneratedAffineResidualCaseAuthority::try_new_direct_formula_singleton(
                &family,
                &context,
            ...
            Err(GeneratedAffineResidualCaseAuthorityError::ResourceLimit {
                ...
                requested,
                limit,
            }) if requested == observed_bytes && limit + 1 == observed_bytes
        ));
    }

    #[test]
    fn direct_formula_singleton_identity_and_premises_remain_allocation_bound() {
        let fixture_name = "direct-case-authority-premise-identity";
        let (family, context, terminals) = direct_sunset_terminals(fixture_name);
        let terminal = Arc::clone(&terminals[0]);
        let authority = Arc::new(
            GeneratedAffineResidualCaseAuthority::try_new_direct_formula_singleton(
                &family,
                &context,
        ...
        let same_terminal_other_authority = Arc::new(
            GeneratedAffineResidualCaseAuthority::try_new_direct_formula_singleton(
                &family,
                &context,
                Arc::clone(&terminal),
        ...
        assert!(authority.same_source_allocation_as(&same_terminal_other_authority));

        let (independent_family, independent_context, independent_terminals) =
            direct_sunset_terminals(fixture_name);
        let independent_authority = Arc::new(
            GeneratedAffineResidualCaseAuthority::try_new_direct_formula_singleton(
                &independent_family,
                &independent_context,
                Arc::clone(&independent_terminals[0]),
                GeneratedAffineResidualCaseAuthorityLimits::default(),
            )
            .unwrap(),
        );
        assert!(!authority.same_source_allocation_as(&independent_authority));
        assert!(!authority.same_direct_formula_terminal_allocation(&independent_terminals[0]));

        let ready = match compile_generated_affine_residual_case_premises(
            &family,
            &context,
            Arc::clone(&authority),
        ...
        .unwrap()
        {
            GeneratedAffineResidualCasePremisesOutcome::Ready(ready) => ready,
            GeneratedAffineResidualCasePremisesOutcome::RequiresAffineEqualityRefinement(_) => {
                panic!("direct actionable terminal unexpectedly requires equality refinement")
            }
        };
        assert_eq!(ready.premises().len(), 1);
        assert_eq!(ready.stats().guard_scans(), 3);
        assert_eq!(ready.stats().nonzero_condition_inputs(), 3);
        assert!(ready.same_authority_allocation(&authority));
        ready
            .replay(&family, &context, &Arc::clone(&authority))
            .unwrap();
        assert!(matches!(
            ready.replay(&family, &context, &same_terminal_other_authority),
            Err(GeneratedAffineResidualCasePremisesError::WrongAuthorityAllocation)
        ));
        assert!(matches!(
            ready.replay(&family, &context, &independent_authority),
            Err(GeneratedAffineResidualCasePremisesError::WrongAuthorityAllocation)
        ));

        let (legacy_family, legacy_context, _, legacy_inventory) = initial_inventory_fixture(
            equal_mass_two_loop_family(fixture_name),
            SectorMask::try_from_bit_string("111").unwrap(),
        );
        assert!(legacy_inventory.case_count() > 0);
        let legacy_authority = Arc::new(
            GeneratedAffineResidualCaseAuthority::try_new(
                &legacy_family,
                &legacy_context,
                Arc::new(legacy_inventory),
                0,
                GeneratedAffineResidualCaseAuthorityLimits::default(),
            )
            .unwrap(),
        ...
        assert!(matches!(
            ready.replay(&family, &context, &legacy_authority),
            Err(GeneratedAffineResidualCasePremisesError::WrongAuthorityAllocation)
        ));
        let legacy_premises = compile_generated_affine_residual_case_premises(
            &legacy_family,
            &legacy_context,
        ...
        .unwrap();
        match legacy_premises {
            GeneratedAffineResidualCasePremisesOutcome::Ready(legacy) => assert!(matches!(
                legacy.replay(&legacy_family, &legacy_context, &authority),
                Err(GeneratedAffineResidualCasePremisesError::WrongAuthorityAllocation)
            )),
        ...
        let fixed_authority = Arc::new(
            GeneratedAffineResidualCaseAuthority::try_new_direct_formula_singleton(
                &family,
                &context,
                Arc::clone(&terminals[5]),
                GeneratedAffineResidualCaseAuthorityLimits::default(),
            )
            .unwrap(),
        );
        let fixed_ready = match compile_generated_affine_residual_case_premises(
            &family,
            &context,
            Arc::clone(&fixed_authority),
            GeneratedAffineResidualCasePremisesLimits::default(),
        )
        .unwrap()
        {
            GeneratedAffineResidualCasePremisesOutcome::Ready(ready) => ready,
            GeneratedAffineResidualCasePremisesOutcome::RequiresAffineEqualityRefinement(_) => {
                panic!("fully fixed direct terminal unexpectedly requires equality refinement")
            }
        };
        assert!(fixed_ready.premises().is_empty());
        assert_eq!(fixed_ready.stats().guard_scans(), 1);
        assert_eq!(fixed_ready.stats().nonzero_condition_inputs(), 0);
        fixed_ready
            .replay(&family, &context, &fixed_authority)
            .unwrap();
    }

    /// Mixed natural recognizer fixture: one active denominator has a
    /// symbolic power shift, the other is ordinary integer-affine, and the
    /// inactive mixed scalar product completes the two-loop coordinate basis.
    ...
    fn initial_inventory_fixture(
        family: IntegralFamily,
        sector: SectorMask,
    ) -> (
        IntegralFamily,
        ParametricCoefficientContext,
        Arc<GeneratedAffineResidualBooleanCoverCertificate>,
        GeneratedAffineResidualCaseInventoryCertificate,
    ) {
        let context = ParametricIbpGenerator::try_new(&family)
            .unwrap()
        ...
        let authority = GeneratedAffineResidualSourceAuthority::initial_global(queue);
        let boolean = Arc::new(
        ...
        let inventory = GeneratedAffineResidualCaseInventoryCompiler::compile(
            &family,
            &context,
            Arc::clone(&boolean),
        ...
        .unwrap();
        (family, context, boolean, inventory)
    }
    ...
    fn one_loop_fixture(
        ...
        initial_inventory_fixture(
            massive_tadpole_family(name),
    ...
    fn two_loop_fixture(
        ...
        initial_inventory_fixture(
            equal_mass_two_loop_family(name),
            SectorMask::try_from_bit_string("011").unwrap(),
        )
    }

    fn shifted_unsupported_fixture(
        name: &str,
    ) -> (
        IntegralFamily,
        ParametricCoefficientContext,
        ...
        initial_inventory_fixture(
            shifted_disconnected_tadpoles(name),
    ...
    fn prior_effective_fixture(
        ...
        let authority = GeneratedAffineResidualSourceAuthority::prior_effective(prior_queue);
        let boolean = Arc::new(
            GeneratedAffineResidualBooleanCoverCompiler::compile(
                &family,
                &context,
    ...
    fn prior_two_loop_fixture(
        ...
        prior_effective_fixture(equal_mass_two_loop_family(name), sector)
    }

    fn prior_shifted_unsupported_fixture(
        name: &str,
    ) -> (
        IntegralFamily,
        ParametricCoefficientContext,
        ...
        prior_effective_fixture(shifted_disconnected_tadpoles(name), "110")
    }

    #[derive(Debug, Default, PartialEq, Eq)]
    struct NaturalTerminalCensus {
        source_proved_empty: usize,
        boolean_proved_empty: usize,
        initial_affine_proved_empty: usize,
        initial_affine_unsupported: usize,
        initial_affine_guard_contradiction: usize,
        initial_affine_actionable: usize,
        prior_unsupported: usize,
        prior_actionable: usize,
        prior_exceptional_domain: usize,
        prior_exceptional_leak: usize,
        initial_equal_zero_atoms: usize,
        initial_nonzero_atoms: usize,
        initial_unsupported_reasons: usize,
        prior_equal_zero_atoms: usize,
        prior_nonzero_atoms: usize,
        prior_unsupported_reasons: usize,
        guard_contradictions: usize,
        guard_discharged_nonzero_integer_constants: usize,
        guard_base_assumptions: usize,
        guard_free_index_dependent: usize,
        exceptional_predicates: usize,
        initial_cases: usize,
        prior_actionable_cases: usize,
        prior_exceptional_domain_cases: usize,
        prior_exceptional_leak_cases: usize,
    }

    fn census_initial_guards(
        guards: ResidualAffineBranchSealedGuardSourceView<'_>,
        census: &mut NaturalTerminalCensus,
    ) {
        for position in 0..guards.guard_count() {
            match guards.guard_entry(position).unwrap().class() {
                ResidualAffineBranchSealedGuardClassSourceView::Contradiction => {
                    census.guard_contradictions += 1;
                }
                ResidualAffineBranchSealedGuardClassSourceView::DischargedNonzeroIntegerConstant => {
                    census.guard_discharged_nonzero_integer_constants += 1;
                }
                ResidualAffineBranchSealedGuardClassSourceView::BaseAssumption { .. } => {
                    census.guard_base_assumptions += 1;
                }
                ResidualAffineBranchSealedGuardClassSourceView::FreeIndexDependent { .. } => {
                    census.guard_free_index_dependent += 1;
                }
            }
        }
    }

    fn census_prior_guard(
        guard: GeneratedAffineResidualBooleanPriorGuardSourceView<'_>,
        census: &mut NaturalTerminalCensus,
    ) {
        match guard.class() {
            GeneratedAffineResidualBooleanPriorGuardClassSourceView::Contradiction => {
    ...
    fn natural_terminal_census(
        context: &ParametricCoefficientContext,
        inventory: &GeneratedAffineResidualCaseInventoryCertificate,
    ) -> NaturalTerminalCensus {
        let mut census = NaturalTerminalCensus::default();
        for ordinal in 0..inventory.terminal_count() {
            match inventory
                .authenticated_terminal_view(context, ordinal)
                .unwrap()
                .source()
            {
                GeneratedAffineResidualInventoryTerminalSourceView::SourceProvedEmpty => {
                    census.source_proved_empty += 1;
                }
                GeneratedAffineResidualInventoryTerminalSourceView::BooleanProvedEmpty(source) => {
                    census.boolean_proved_empty += 1;
                    census.initial_equal_zero_atoms += source.equal_zero_atom_count();
                    census.initial_nonzero_atoms += source.nonzero_atom_count();
                }
                GeneratedAffineResidualInventoryTerminalSourceView::InitialAffineProvedEmpty(_) => {
                    census.initial_affine_proved_empty += 1;
                }
                GeneratedAffineResidualInventoryTerminalSourceView::InitialAffineUnsupported(source) => {
                    census.initial_affine_unsupported += 1;
                    census.initial_unsupported_reasons += source.reason_count();
                }
                GeneratedAffineResidualInventoryTerminalSourceView::InitialAffineGuardContradiction(source) => {
                    census.initial_affine_guard_contradiction += 1;
                    census_initial_guards(source.guards(), &mut census);
                }
                GeneratedAffineResidualInventoryTerminalSourceView::InitialAffineActionable(source) => {
                    census.initial_affine_actionable += 1;
                    census_initial_guards(source.guards(), &mut census);
                }
                GeneratedAffineResidualInventoryTerminalSourceView::PriorUnsupported(source) => {
                    census.prior_unsupported += 1;
                    census.prior_equal_zero_atoms +=
                        source.atom_count(GeneratedAffineResidualBooleanAtomPolarity::EqualZero);
                    census.prior_nonzero_atoms +=
                        source.atom_count(GeneratedAffineResidualBooleanAtomPolarity::NonZero);
                    census.prior_unsupported_reasons += source.unsupported_reason_count();
                }
                GeneratedAffineResidualInventoryTerminalSourceView::PriorActionable(source) => {
                    census.prior_actionable += 1;
                    for position in 0..source.guard_entry_count() {
                        census_prior_guard(source.guard_entry(position).unwrap(), &mut census);
                    }
                }
                GeneratedAffineResidualInventoryTerminalSourceView::PriorExceptionalDomain(source) => {
                    census.prior_exceptional_domain += 1;
                    census.exceptional_predicates += source.predicate_count();
                    for position in 0..source.guard_entry_count() {
                        census_prior_guard(source.guard_entry(position).unwrap(), &mut census);
                    }
                }
                GeneratedAffineResidualInventoryTerminalSourceView::PriorExceptionalLeak(source) => {
                    census.prior_exceptional_leak += 1;
                    census.exceptional_predicates += source.predicate_count();
                    for position in 0..source.guard_entry_count() {
                        census_prior_guard(source.guard_entry(position).unwrap(), &mut census);
                    }
        ...
        for ordinal in 0..inventory.case_count() {
            match inventory
                ...
                .source()
            {
                GeneratedAffineResidualInventoryCaseSourceView::Initial(_) => {
                    census.initial_cases += 1;
                }
                GeneratedAffineResidualInventoryCaseSourceView::PriorActionable(_) => {
                    census.prior_actionable_cases += 1;
                }
                GeneratedAffineResidualInventoryCaseSourceView::PriorExceptionalDomain(_) => {
                    census.prior_exceptional_domain_cases += 1;
                }
                GeneratedAffineResidualInventoryCaseSourceView::PriorExceptionalLeak(_) => {
                    census.prior_exceptional_leak_cases += 1;
                }
            }
        }
        census
    ...
    fn case_for_terminal<'source>(
        context: &ParametricCoefficientContext,
        inventory: &'source GeneratedAffineResidualCaseInventoryCertificate,
        terminal_ordinal: usize,
    ) -> GeneratedAffineResidualInventoryCaseSourceRecordView<'source> {
        let mut matches = (0..inventory.case_count()).filter_map(|case_ordinal| {
            let case = inventory
                .authenticated_case_view(context, case_ordinal)
                .unwrap();
            (case.terminal_ordinal() == terminal_ordinal).then_some(case)
        });
        let case = matches
            .next()
            .expect("actionable terminal has one retained case");
        assert!(
            matches.next().is_none(),
            "terminal cannot bind more than one retained case"
        );
        case
    }

    fn exact_integer_map_witness(map: &ResidualAffineIntegerMap, sector: &SectorMask) -> Vec<i64> {
        let seed = sector
            .active_bits()
            .iter()
        ...
        (0..map.ambient_arity())
            .map(|row| {
                ...
                image
                    .to_i64()
                    .expect("small natural fixture witness fits in i64")
            })
            .collect()
    }

    fn case_full_point_chain_witness(
        family: &IntegralFamily,
        context: &ParametricCoefficientContext,
        inventory: &GeneratedAffineResidualCaseInventoryCertificate,
        case: GeneratedAffineResidualInventoryCaseSourceRecordView<'_>,
    ) -> Option<Vec<i64>> {
        fn bounded_case_witness(
            position: usize,
            active_bits: &[bool],
            point: &mut [i64],
            matches: &mut impl FnMut(&[i64]) -> bool,
        ) -> bool {
            if position == point.len() {
                return matches(point);
            }
            let range = if active_bits[position] { 1..=6 } else { -4..=0 };
            for value in range {
                point[position] = value;
                if bounded_case_witness(position + 1, active_bits, point, matches) {
                    return true;
                }
            }
            false
        ...
        let expected = GeneratedAffineResidualInventoryPointDisposition::Actionable {
            terminal_ordinal: case.terminal_ordinal(),
            case_ordinal: case.ordinal(),
        };
        let mut classify = |point: &[i64]| {
            inventory
                .classification_for_indices(
                    family,
                    context,
                    point,
                    GeneratedAffineResidualInventoryPointLimits::default(),
                )
                .unwrap()
                .disposition()
                == expected
        };
        let map_witness = exact_integer_map_witness(case.source().affine_map(), inventory.sector());
        let point = if classify(&map_witness) {
            map_witness
        } else {
            let mut point = vec![0; inventory.arity()];
            if !bounded_case_witness(
                0,
                inventory.sector().active_bits(),
                &mut point,
                &mut classify,
            ) {
                return None;
        ...
        assert_eq!(
            ...
            expected,
            "map-derived point {point:?} must traverse the full source/Boolean/inventory chain"
        );
        assert_eq!(
            classification
                .stats()
    ...
    fn exact_residual_point_specialization_limits(
    ...
    fn exact_owner_point_specialization_limits(
    ...
    fn exact_owner_point_limits(
        ...
        GeneratedSectorAffinePointLimits {
            map: stats
                .map()
                .map_or_else(ResidualAffineIntegerMapPointLimits::default, |map| {
                    ResidualAffineIntegerMapPointLimits {
                        max_ambient_arity: map.ambient_arity(),
                        max_matrix_entries_inspected: map.matrix_entries_inspected(),
                        max_nonzero_multiplications: map.nonzero_multiplications(),
                        max_additions: map.additions(),
                        max_fixed_point_comparisons: map.fixed_point_comparisons(),
                        max_peak_temporary_bytes: map.peak_temporary_bytes(),
                        max_integer_bits: map.largest_integer_bits(),
                        max_integer_bit_work: map.integer_bit_work(),
                    }
                }),
            relative: stats.relative().map_or_else(
                GeneratedResidualAffineWhenBadPointLimits::default,
                exact_relative_point_limits,
            ),
            global_specialization: exact_owner_point_specialization_limits(
                stats.global_specialization(),
            ),
            boolean_specialization: exact_owner_point_specialization_limits(
                stats.boolean_specialization(),
            ),
    ...
    fn exact_source_point_limits(
        ...
        prior_effective.max_work_item_scans = stats.work_item_scans();
        if let Some(owner) = stats.prior_effective_owner() {
            prior_effective.owner = exact_owner_point_limits(owner);
        }
        GeneratedAffineResidualSourcePointLimits {
            prior_effective,
            initial_specialization: exact_residual_point_specialization_limits(
            ...
            max_initial_work_item_scans: stats.work_item_scans(),
            max_initial_disposition_candidate_comparisons: stats
                .initial_disposition_candidate_comparisons(),
        }
    }

    fn exact_initial_boolean_point_limits(
        stats: GeneratedAffineInitialGlobalBooleanPointStats,
    ) -> GeneratedAffineInitialGlobalBooleanPointLimits {
        GeneratedAffineInitialGlobalBooleanPointLimits {
            specialization: exact_residual_point_specialization_limits(stats.specialization()),
            max_context_comparison_bytes: stats.context_comparison_bytes(),
            max_sector_index_scans: stats.sector_index_scans(),
            max_node_scans: stats.node_scans(),
            max_terminal_scans: stats.terminal_scans(),
            max_ready_terminal_scans: stats.ready_terminal_scans(),
            max_atom_scans: stats.atom_scans(),
            max_atom_evaluations: stats.atom_evaluations(),
        }
    }
    ...
    fn exact_boolean_point_limits(
        ...
        let navigation = stats.source_navigation();
        GeneratedAffineResidualBooleanPointLimits {
            source: exact_source_point_limits(stats.source()),
            source_navigation: GeneratedAffineResidualSourceNavigationLimits {
                max_source_view_resolutions: navigation.source_view_resolutions(),
                max_initial_case_lookup_comparisons: navigation.initial_case_lookup_comparisons(),
                max_initial_disposition_candidate_comparisons: navigation
                    .initial_disposition_candidate_comparisons(),
                max_prior_authority_index_comparison_bound: navigation
                    .prior_authority_index_comparison_bound(),
                max_prior_projection_payload_comparison_bound: navigation
                    .prior_projection_payload_comparison_bound(),
            },
            initial_boolean: exact_initial_boolean_point_limits(stats.initial_boolean()),
            max_terminal_record_scans: stats.terminal_record_scans(),
        }
    }

    fn assert_inventory_delegated_boolean_every_positive_one_below(
        family: &IntegralFamily,
        context: &ParametricCoefficientContext,
        inventory: &GeneratedAffineResidualCaseInventoryCertificate,
        indices: &[i64],
        stats: GeneratedAffineResidualBooleanPointStats,
        exact: GeneratedAffineResidualInventoryPointLimits,
    ) {
        macro_rules! reject {
            ($limits:expr) => {
                assert!(matches!(
                    inventory.classification_for_indices(family, context, indices, $limits),
                    Err(GeneratedAffineResidualInventoryPointError::Boolean(_))
                ));
            };
        }
        macro_rules! one_below {
            ($path:expr, $observed:expr) => {{
                let observed = $observed;
                if observed > 0 {
                    let mut limits = exact;
                    $path(&mut limits, observed - 1);
                    reject!(limits);
                }
            }};
        }
        ...
        one_below!(
            |limits: &mut GeneratedAffineResidualInventoryPointLimits, value| {
                limits.boolean.max_terminal_record_scans = value
            },
            stats.terminal_record_scans()
        );
        let navigation = stats.source_navigation();
        one_below!(
            |limits: &mut GeneratedAffineResidualInventoryPointLimits, value| {
                limits.boolean.source_navigation.max_source_view_resolutions = value
            },
            navigation.source_view_resolutions()
        );
        one_below!(
            |limits: &mut GeneratedAffineResidualInventoryPointLimits, value| {
                limits
                    .boolean
                    .source_navigation
                    .max_initial_case_lookup_comparisons = value
            },
            navigation.initial_case_lookup_comparisons()
        );
        one_below!(
            |limits: &mut GeneratedAffineResidualInventoryPointLimits, value| {
                limits
                    .boolean
                    .source_navigation
                    .max_initial_disposition_candidate_comparisons = value
            },
            navigation.initial_disposition_candidate_comparisons()
        );
        one_below!(
            |limits: &mut GeneratedAffineResidualInventoryPointLimits, value| {
                limits
                    .boolean
                    .source_navigation
                    .max_prior_authority_index_comparison_bound = value
            },
        ...
        one_below!(
            |limits: &mut GeneratedAffineResidualInventoryPointLimits, value| {
                limits
                    .boolean
                    .source_navigation
                    .max_prior_projection_payload_comparison_bound = value
            },
            navigation.prior_projection_payload_comparison_bound()
        );

        let source = stats.source();
        macro_rules! source_outer {
            ($field:ident, $getter:ident) => {
                one_below!(
                    |limits: &mut GeneratedAffineResidualInventoryPointLimits, value| {
                        limits.boolean.source.$field = value
                    },
                    source.$getter()
                );
            };
        }
        source_outer!(max_scope_comparison_bytes, scope_comparison_bytes);
        source_outer!(max_index_entries, index_entries);
        source_outer!(max_initial_orthant_index_scans, initial_orthant_index_scans);
        source_outer!(max_initial_case_scans, initial_case_scans);
        source_outer!(
            max_initial_classification_scans,
            initial_classification_scans
        );
        source_outer!(max_initial_predicate_scans, initial_predicate_scans);
        source_outer!(
        ...
        match source.kind() {
            Some(GeneratedAffineResidualSourceAuthorityKind::InitialGlobal) => {
                source_outer!(max_initial_work_item_scans, work_item_scans);
            }
            Some(GeneratedAffineResidualSourceAuthorityKind::PriorEffective) => {
                one_below!(
                    |limits: &mut GeneratedAffineResidualInventoryPointLimits, value| {
                        limits.boolean.source.prior_effective.max_work_item_scans = value
                    },
                    source.work_item_scans()
                );
            }
            None => {}
        }

        let initial_specialization = source.initial_specialization();
        macro_rules! source_specialization {
            ($field:ident, $getter:ident) => {
                one_below!(
                    |limits: &mut GeneratedAffineResidualInventoryPointLimits, value| {
                        limits.boolean.source.initial_specialization.$field = value
                    },
                    initial_specialization.$getter()
                );
            };
        }
        source_specialization!(max_source_terms, source_terms);
        source_specialization!(max_source_exponent_entries, source_exponent_entries);
        ...
        source_specialization!(max_output_term_bound, output_term_bound);
        source_specialization!(max_output_exponent_entry_bound, output_exponent_entry_bound);
        source_specialization!(max_power_operation_bound, power_operation_bound);
        source_specialization!(
            max_largest_output_integer_bit_bound,
            largest_output_integer_bit_bound
        );
        source_specialization!(max_integer_bit_work_bound, integer_bit_work_bound);
        source_specialization!(max_retained_output_term_bound, retained_output_term_bound);
        source_specialization!(max_retained_output_byte_bound, retained_output_byte_bound);

        let initial_boolean = stats.initial_boolean();
        macro_rules! initial_boolean_outer {
            ($field:ident, $getter:ident) => {
                one_below!(
                    |limits: &mut GeneratedAffineResidualInventoryPointLimits, value| {
                        limits.boolean.initial_boolean.$field = value
                    },
                    initial_boolean.$getter()
                );
            };
        }
        initial_boolean_outer!(max_context_comparison_bytes, context_comparison_bytes);
        initial_boolean_outer!(max_sector_index_scans, sector_index_scans);
        initial_boolean_outer!(max_node_scans, node_scans);
        initial_boolean_outer!(max_terminal_scans, terminal_scans);
        initial_boolean_outer!(max_ready_terminal_scans, ready_terminal_scans);
        initial_boolean_outer!(max_atom_scans, atom_scans);
        initial_boolean_outer!(max_atom_evaluations, atom_evaluations);
        let initial_boolean_specialization = initial_boolean.specialization();
        macro_rules! initial_boolean_specialization {
            ($field:ident, $getter:ident) => {
                one_below!(
                    |limits: &mut GeneratedAffineResidualInventoryPointLimits, value| {
                        limits.boolean.initial_boolean.specialization.$field = value
                    },
                    initial_boolean_specialization.$getter()
                );
            };
        }
        initial_boolean_specialization!(max_source_terms, source_terms);
        initial_boolean_specialization!(max_source_exponent_entries, source_exponent_entries);
        initial_boolean_specialization!(
            max_preflight_validation_source_term_scan_bound,
            preflight_validation_source_term_scan_bound
        );
        initial_boolean_specialization!(
        ...
        initial_boolean_specialization!(max_output_term_bound, output_term_bound);
        initial_boolean_specialization!(
            max_output_exponent_entry_bound,
            output_exponent_entry_bound
        );
        initial_boolean_specialization!(max_power_operation_bound, power_operation_bound);
        initial_boolean_specialization!(
            max_largest_output_integer_bit_bound,
            largest_output_integer_bit_bound
        );
        initial_boolean_specialization!(max_integer_bit_work_bound, integer_bit_work_bound);
        initial_boolean_specialization!(max_retained_output_term_bound, retained_output_term_bound);
        initial_boolean_specialization!(max_retained_output_byte_bound, retained_output_byte_bound);
        ...
        if let Some(owner) = source.prior_effective_owner() {
            macro_rules! owner_outer {
                ($field:ident, $getter:ident) => {
                    one_below!(
                        |limits: &mut GeneratedAffineResidualInventoryPointLimits, value| {
                            limits.boolean.source.prior_effective.owner.$field = value
                        },
                        owner.$getter()
                    );
                };
            ...
            macro_rules! owner_specialization {
                ($stage:ident, $stats:expr, $field:ident, $getter:ident) => {
                    one_below!(
                        |limits: &mut GeneratedAffineResidualInventoryPointLimits, value| {
                            limits.boolean.source.prior_effective.owner.$stage.$field = value
                        },
                        $stats.$getter()
                    );
                };
            }
            macro_rules! all_owner_specialization {
                ($stage:ident, $stage_stats:expr) => {
                    owner_specialization!($stage, $stage_stats, max_source_terms, source_terms);
            ...
            all_owner_specialization!(global_specialization, owner.global_specialization());
            all_owner_specialization!(boolean_specialization, owner.boolean_specialization());

            if let Some(map) = owner.map() {
                macro_rules! owner_map {
                    ($field:ident, $getter:ident) => {
                        one_below!(
                            |limits: &mut GeneratedAffineResidualInventoryPointLimits, value| {
                                limits.boolean.source.prior_effective.owner.map.$field = value
                            },
                            map.$getter()
                        );
                    };
                }
                owner_map!(max_ambient_arity, ambient_arity);
                owner_map!(max_matrix_entries_inspected, matrix_entries_inspected);
                owner_map!(max_nonzero_multiplications, nonzero_multiplications);
                owner_map!(max_additions, additions);
                owner_map!(max_fixed_point_comparisons, fixed_point_comparisons);
                owner_map!(max_peak_temporary_bytes, peak_temporary_bytes);
                owner_map!(max_integer_bits, largest_integer_bits);
                owner_map!(max_integer_bit_work, integer_bit_work);
            }
            if let Some(relative) = owner.relative() {
                macro_rules! owner_relative {
                    ($field:ident, $getter:ident) => {
                        one_below!(
                            |limits: &mut GeneratedAffineResidualInventoryPointLimits, value| {
                                limits.boolean.source.prior_effective.owner.relative.$field = value
                            },
                            relative.$getter()
                        );
                    };
                ...
                owner_relative!(max_index_entries, index_entries);
                owner_relative!(max_cases, cases);
                owner_relative!(max_classifications, classifications);
                owner_relative!(max_predicates, predicates);
                owner_relative!(max_source_terms, source_terms);
                owner_relative!(max_source_exponent_entries, source_exponent_entries);
                owner_relative!(
                    max_preflight_validation_source_term_scan_bound,
                    preflight_validation_source_term_scan_bound
                );
                ...
                owner_relative!(max_output_term_bound, output_term_bound);
                owner_relative!(max_output_exponent_entry_bound, output_exponent_entry_bound);
                owner_relative!(max_power_operation_bound, power_operation_bound);
                owner_relative!(
                    max_largest_output_integer_bit_bound,
                    largest_output_integer_bit_bound
                );
                owner_relative!(max_integer_bit_work_bound, integer_bit_work_bound);
                owner_relative!(max_retained_output_term_bound, retained_output_term_bound);
                owner_relative!(max_retained_output_byte_bound, retained_output_byte_bound);
            }
        }
    }

    fn assert_inventory_point_exact_and_every_positive_one_below(
        family: &IntegralFamily,
        context: &ParametricCoefficientContext,
        inventory: &GeneratedAffineResidualCaseInventoryCertificate,
        indices: &[i64],
        exact: GeneratedAffineResidualInventoryPointClassification,
    ) {
        let stats = exact.stats();
        let authentication = stats.authentication();
        let mut exact_limits = GeneratedAffineResidualInventoryPointLimits::default();
        exact_limits.boolean = exact_boolean_point_limits(stats.boolean());
        exact_limits.max_case_scans = stats.case_scans();
        exact_limits.authentication.max_terminal_authentications =
            authentication.terminal_authentications();
        exact_limits.authentication.max_terminal_payload_references =
            authentication.terminal_payload_references();
        exact_limits
            .authentication
            .max_initial_affine_projection_authentications =
            authentication.initial_affine_projection_authentications();
        exact_limits
            .authentication
            .max_initial_affine_ready_binding_units =
            authentication.initial_affine_ready_binding_units();
        exact_limits
            .authentication
            .max_initial_affine_ready_binding_bytes =
            authentication.initial_affine_ready_binding_bytes();
        exact_limits
            .authentication
            .max_initial_affine_child_retained_owned_logical_bytes =
            authentication.initial_affine_child_retained_owned_logical_bytes();
        exact_limits.authentication.max_case_authentications =
            authentication.case_authentications();
        exact_limits.authentication.max_group_authentications =
            authentication.group_authentications();
        exact_limits.authentication.max_group_case_references =
            authentication.group_case_references();
        exact_limits.authentication.max_geometry_shape_comparisons =
            authentication.geometry_shape_comparisons();
        exact_limits
            .authentication
            .max_geometry_constant_comparisons = authentication.geometry_constant_comparisons();
        exact_limits
            .authentication
            .max_geometry_free_position_comparisons =
            authentication.geometry_free_position_comparisons();
        exact_limits
            .authentication
            .max_geometry_compact_matrix_comparisons =
            authentication.geometry_compact_matrix_comparisons();
        exact_limits
            .authentication
            .max_geometry_anchor_offset_comparisons =
            authentication.geometry_anchor_offset_comparisons();
        exact_limits
            .authentication
            .max_geometry_integer_bit_inspections =
            authentication.geometry_integer_bit_inspections();
        exact_limits.authentication.max_geometry_integer_bit_work =
            authentication.geometry_integer_bit_work();
        exact_limits
            .authentication
            .max_geometry_peak_gmp_logical_bytes = authentication.geometry_peak_gmp_logical_bytes();
        if let Some(map) = stats.affine_map() {
            exact_limits.affine_map.max_ambient_arity = map.ambient_arity();
            exact_limits.affine_map.max_matrix_entries_inspected = map.matrix_entries_inspected();
            exact_limits.affine_map.max_nonzero_multiplications = map.nonzero_multiplications();
            exact_limits.affine_map.max_additions = map.additions();
            exact_limits.affine_map.max_fixed_point_comparisons = map.fixed_point_comparisons();
            exact_limits.affine_map.max_peak_temporary_bytes = map.peak_temporary_bytes();
            exact_limits.affine_map.max_integer_bits = map.largest_integer_bits();
            exact_limits.affine_map.max_integer_bit_work = map.integer_bit_work();
        }
        assert_eq!(
            inventory
                .classification_for_indices(family, context, indices, exact_limits)
                .unwrap()
                .disposition(),
            exact.disposition()
        );

        assert_inventory_delegated_boolean_every_positive_one_below(
            family,
            context,
            inventory,
            indices,
        ...
        macro_rules! authentication_one_below {
            ($limit:ident, $stat:ident) => {
                if authentication.$stat() > 0 {
                    let mut one_below = exact_limits;
                    one_below.authentication.$limit = authentication.$stat() - 1;
                    assert!(matches!(
                        inventory.classification_for_indices(family, context, indices, one_below,),
                        Err(GeneratedAffineResidualInventoryPointError::ResourceLimit { .. })
                    ));
                }
            };
        }
        authentication_one_below!(max_terminal_authentications, terminal_authentications);
        authentication_one_below!(max_terminal_payload_references, terminal_payload_references);
        authentication_one_below!(
            max_initial_affine_projection_authentications,
            initial_affine_projection_authentications
        );
        authentication_one_below!(
        ...
        authentication_one_below!(max_case_authentications, case_authentications);
        authentication_one_below!(max_group_authentications, group_authentications);
        authentication_one_below!(max_group_case_references, group_case_references);
        authentication_one_below!(max_geometry_shape_comparisons, geometry_shape_comparisons);
        authentication_one_below!(
            max_geometry_constant_comparisons,
            geometry_constant_comparisons
        );
        authentication_one_below!(
            max_geometry_free_position_comparisons,
        ...
        authentication_one_below!(max_geometry_integer_bit_work, geometry_integer_bit_work);
        authentication_one_below!(
            max_geometry_peak_gmp_logical_bytes,
            geometry_peak_gmp_logical_bytes
        );

        if stats.case_scans() > 0 {
            let mut one_below = exact_limits;
            one_below.max_case_scans = stats.case_scans() - 1;
            assert!(matches!(
                inventory.classification_for_indices(family, context, indices, one_below),
                Err(GeneratedAffineResidualInventoryPointError::ResourceLimit {
                    resource: "case scans",
        ...
        if let Some(map) = stats.affine_map() {
            macro_rules! map_one_below {
                ($limit:ident, $stat:ident) => {
                    if map.$stat() > 0 {
                        let mut one_below = exact_limits;
                        one_below.affine_map.$limit = map.$stat() - 1;
                        assert!(matches!(
                            inventory
                                .classification_for_indices(family, context, indices, one_below,),
                            Err(GeneratedAffineResidualInventoryPointError::AffineMap(
    ...
    fn exact_inventory_authentication_limits(
        stats: GeneratedAffineResidualInventoryAuthenticationStats,
    ) -> GeneratedAffineResidualInventoryAuthenticationLimits {
        let mut limits = GeneratedAffineResidualInventoryAuthenticationLimits::default();
        limits.max_terminal_authentications = stats.terminal_authentications();
        limits.max_terminal_payload_references = stats.terminal_payload_references();
        limits.max_initial_affine_projection_authentications =
            stats.initial_affine_projection_authentications();
        limits.max_initial_affine_ready_binding_units = stats.initial_affine_ready_binding_units();
        limits.max_initial_affine_ready_binding_bytes = stats.initial_affine_ready_binding_bytes();
        limits.max_initial_affine_child_retained_owned_logical_bytes =
            stats.initial_affine_child_retained_owned_logical_bytes();
        limits.max_case_authentications = stats.case_authentications();
        limits.max_group_authentications = stats.group_authentications();
        limits.max_group_case_references = stats.group_case_references();
        limits.max_geometry_shape_comparisons = stats.geometry_shape_comparisons();
        limits.max_geometry_constant_comparisons = stats.geometry_constant_comparisons();
        limits.max_geometry_free_position_comparisons = stats.geometry_free_position_comparisons();
        limits.max_geometry_compact_matrix_comparisons =
            stats.geometry_compact_matrix_comparisons();
        limits.max_geometry_anchor_offset_comparisons = stats.geometry_anchor_offset_comparisons();
        limits.max_geometry_integer_bit_inspections = stats.geometry_integer_bit_inspections();
        limits.max_geometry_integer_bit_work = stats.geometry_integer_bit_work();
        limits.max_geometry_peak_gmp_logical_bytes = stats.geometry_peak_gmp_logical_bytes();
        limits
    }

    fn exact_group_boolean_source_navigation_limits(
        inventory: &GeneratedAffineResidualCaseInventoryCertificate,
        group_ordinal: usize,
    ) -> GeneratedAffineResidualSourceNavigationLimits {
        let group = inventory
            .groups
            .get(group_ordinal)
            .expect("authenticated group ordinal is retained");
        let mut exact = GeneratedAffineResidualSourceNavigationLimits {
            max_source_view_resolutions: 0,
            max_initial_case_lookup_comparisons: 0,
            max_initial_disposition_candidate_comparisons: 0,
            max_prior_authority_index_comparison_bound: 0,
            max_prior_projection_payload_comparison_bound: 0,
        };
        for &case_ordinal in &group.case_ordinals {
            let terminal_ordinal = inventory
                .cases
                .get(case_ordinal)
                .expect("authenticated group case is retained")
                .terminal_ordinal;
            let (_, stats) = inventory
                .source_boolean_cover
                .authenticated_terminal_view_with_limits(
                    terminal_ordinal,
                    GeneratedAffineResidualSourceNavigationLimits::default(),
                )
                .expect("retained inventory case resolves through its exact Boolean source");
            exact.max_source_view_resolutions = exact
                .max_source_view_resolutions
                .max(stats.source_view_resolutions());
            exact.max_initial_case_lookup_comparisons = exact
                .max_initial_case_lookup_comparisons
                .max(stats.initial_case_lookup_comparisons());
            exact.max_initial_disposition_candidate_comparisons = exact
                .max_initial_disposition_candidate_comparisons
                .max(stats.initial_disposition_candidate_comparisons());
            exact.max_prior_authority_index_comparison_bound = exact
                .max_prior_authority_index_comparison_bound
                .max(stats.prior_authority_index_comparison_bound());
            exact.max_prior_projection_payload_comparison_bound = exact
                .max_prior_projection_payload_comparison_bound
                .max(stats.prior_projection_payload_comparison_bound());
        }
        exact
    }

    fn assert_inventory_group_exact_and_every_positive_one_below(
        context: &ParametricCoefficientContext,
        inventory: &GeneratedAffineResidualCaseInventoryCertificate,
        group_ordinal: usize,
    ) -> GeneratedAffineResidualInventoryAuthenticationStats {
        ...
        inventory
            ...
            .unwrap();
        let exact_navigation =
            exact_group_boolean_source_navigation_limits(inventory, group_ordinal);
        let mut exact_limits = exact_inventory_authentication_limits(stats);
        exact_limits.boolean_source_navigation = exact_navigation;
        let mut exact_stats = GeneratedAffineResidualInventoryAuthenticationStats::default();
        inventory
            .authenticated_group_view_with_inventory_limits(
                context,
                group_ordinal,
        ...
        macro_rules! one_below {
            ($limit:ident, $stat:ident) => {
                if stats.$stat() > 0 {
                    ...
                    limits.$limit = stats.$stat() - 1;
                    let mut rejected_stats =
                        GeneratedAffineResidualInventoryAuthenticationStats::default();
                    assert!(matches!(
                        inventory.authenticated_group_view_with_inventory_limits(
                            context,
                            group_ordinal,
                            limits,
                            &mut rejected_stats,
        ...
        one_below!(max_terminal_authentications, terminal_authentications);
        one_below!(max_terminal_payload_references, terminal_payload_references);
        one_below!(
            max_initial_affine_projection_authentications,
            initial_affine_projection_authentications
        );
        ...
        one_below!(max_case_authentications, case_authentications);
        one_below!(max_group_authentications, group_authentications);
        one_below!(max_group_case_references, group_case_references);
        one_below!(max_geometry_shape_comparisons, geometry_shape_comparisons);
        one_below!(
            max_geometry_constant_comparisons,
            geometry_constant_comparisons
        );
        ...
        macro_rules! navigation_one_below {
            ($limit:ident) => {
                if exact_navigation.$limit > 0 {
                    let mut limits = exact_limits;
                    limits.boolean_source_navigation.$limit = exact_navigation.$limit - 1;
                    let mut rejected_stats =
                        GeneratedAffineResidualInventoryAuthenticationStats::default();
                    assert!(matches!(
                        inventory.authenticated_group_view_with_inventory_limits(
        ...
        navigation_one_below!(max_source_view_resolutions);
        navigation_one_below!(max_initial_case_lookup_comparisons);
        navigation_one_below!(max_initial_disposition_candidate_comparisons);
        navigation_one_below!(max_prior_authority_index_comparison_bound);
        navigation_one_below!(max_prior_projection_payload_comparison_bound);
        stats
    }

    fn exact_case_authority_limits(
        stats: GeneratedAffineResidualCaseAuthorityStats,
        boolean_source_navigation: GeneratedAffineResidualSourceNavigationLimits,
    ) -> GeneratedAffineResidualCaseAuthorityLimits {
        let mut limits = GeneratedAffineResidualCaseAuthorityLimits::default();
        limits.max_scope_comparison_bytes = stats.scope_comparison_bytes();
        limits.max_inventory_replays = stats.inventory_replays();
        limits.max_replay_terminals = stats.replay_terminals();
        limits.max_replay_cases = stats.replay_cases();
        limits.max_replay_groups = stats.replay_groups();
        limits.max_replay_group_shape_scans = stats.replay_group_shape_scans();
        limits.max_replay_group_case_references = stats.replay_group_case_references();
        limits.max_replay_payload_comparison_units = stats.replay_payload_comparison_units();
        limits.max_replay_payload_comparison_bytes = stats.replay_payload_comparison_bytes();
        limits.max_replay_payload_comparison_integer_bits =
            stats.replay_payload_comparison_integer_bits();
        limits.max_replay_recursive_child_comparison_units =
            stats.replay_recursive_child_comparison_units();
        limits.max_replay_recursive_child_comparison_bytes =
            stats.replay_recursive_child_comparison_bytes();
        limits.max_replay_recursive_child_comparison_integer_bits =
            stats.replay_recursive_child_comparison_integer_bits();
        limits.max_replay_owned_logical_peak = stats.replay_owned_logical_peak();
        limits.authentication = exact_inventory_authentication_limits(stats.authentication());
        limits.authentication.boolean_source_navigation = boolean_source_navigation;
        limits
    }

    fn assert_case_authority_exact_and_every_positive_replay_one_below(
        family: &IntegralFamily,
        context: &ParametricCoefficientContext,
        inventory: &Arc<GeneratedAffineResidualCaseInventoryCertificate>,
        case_ordinal: usize,
        baseline: &GeneratedAffineResidualCaseAuthority,
    ) {
        let stats = baseline.stats();
        let exact_navigation = exact_group_boolean_source_navigation_limits(
            inventory.as_ref(),
            baseline.group_ordinal(),
        );
        let exact_limits = exact_case_authority_limits(stats, exact_navigation);
        let exact = GeneratedAffineResidualCaseAuthority::try_new(
            family,
            context,
            Arc::clone(inventory),
            case_ordinal,
        ...
        one_below!(max_scope_comparison_bytes, scope_comparison_bytes);
        one_below!(max_inventory_replays, inventory_replays);
        one_below!(max_replay_terminals, replay_terminals);
        one_below!(max_replay_cases, replay_cases);
        one_below!(max_replay_groups, replay_groups);
        one_below!(max_replay_group_shape_scans, replay_group_shape_scans);
        one_below!(
            max_replay_group_case_references,
            replay_group_case_references
        );
        one_below!(
            max_replay_payload_comparison_units,
            replay_payload_comparison_units
        );
        one_below!(
            max_replay_payload_comparison_bytes,
            replay_payload_comparison_bytes
        );
        one_below!(
            max_replay_payload_comparison_integer_bits,
            replay_payload_comparison_integer_bits
        );
        one_below!(
            max_replay_recursive_child_comparison_units,
            replay_recursive_child_comparison_units
        );
        one_below!(
            max_replay_recursive_child_comparison_bytes,
            replay_recursive_child_comparison_bytes
        );
        one_below!(
            max_replay_recursive_child_comparison_integer_bits,
            replay_recursive_child_comparison_integer_bits
        );
        one_below!(max_replay_owned_logical_peak, replay_owned_logical_peak);

        let authentication = stats.authentication();
        macro_rules! authentication_one_below {
            ($limit:ident, $stat:ident, $resource:literal) => {
                if authentication.$stat() > 0 {
                    let requested = authentication.$stat();
                    let mut limits = exact_limits;
                    limits.authentication.$limit = requested - 1;
                    assert!(matches!(
                        GeneratedAffineResidualCaseAuthority::try_new(
                            family,
                            context,
        ...
        authentication_one_below!(
            max_terminal_authentications,
            terminal_authentications,
            "terminal authentications"
        );
        authentication_one_below!(
            max_terminal_payload_references,
            terminal_payload_references,
            "terminal payload references"
        );
        authentication_one_below!(
        ...
        authentication_one_below!(
            max_case_authentications,
            case_authentications,
            "case authentications"
        );
        authentication_one_below!(
            max_group_authentications,
            group_authentications,
            "group authentications"
        );
        authentication_one_below!(
            max_group_case_references,
            group_case_references,
            "group case references"
        );
        authentication_one_below!(
        ...
        macro_rules! navigation_one_below {
            ($limit:ident) => {
                if exact_navigation.$limit > 0 {
                    let mut limits = exact_limits;
                    limits.authentication.boolean_source_navigation.$limit =
                        exact_navigation.$limit - 1;
                    assert!(matches!(
                        GeneratedAffineResidualCaseAuthority::try_new(
                            family,
                            context,
    ...
    fn assert_source_row_exact_and_every_positive_one_below(
        ...
        let baseline = authority
            ...
            .unwrap();
        let stats = baseline.stats();
        let exact_limits = GeneratedAffineResidualCaseSourceRowLimits {
            max_scope_comparison_bytes: stats.scope_comparison_bytes(),
            max_source_rows: stats.source_rows(),
            max_relation_terms: stats.relation_terms(),
            max_guard_conditions: stats.guard_conditions(),
        };
        let exact = authority
            .authenticated_source_row_view(family, context, source_row_ordinal, exact_limits)
            .unwrap();
        assert_eq!(exact.stats(), stats);
        assert!(std::ptr::eq(exact.relation(), baseline.relation()));
                ...
                if stats.$stat() > 0 {
                    ...
                    assert!(matches!(
                        ...
                        Err(GeneratedAffineResidualCaseAuthorityError::ResourceLimit {
                            ...
                            requested,
                            limit,
                        }) if requested == stats.$stat() && limit == stats.$stat() - 1
                    ));
                }
            };
        }
        ...
        one_below!(max_source_rows, source_rows, "source rows");
        one_below!(max_relation_terms, relation_terms, "source relation terms");
        one_below!(
            max_guard_conditions,
            guard_conditions,
            "source relation guard conditions"
        );
    }

    fn exact_same_group_target_cases_limits(
        stats: GeneratedAffineResidualSameGroupTargetCasesStats,
    ) -> GeneratedAffineResidualSameGroupTargetCasesLimits {
        GeneratedAffineResidualSameGroupTargetCasesLimits {
            max_scope_comparison_bytes: stats.scope_comparison_bytes(),
            max_case_lookups: stats.case_lookups(),
            max_group_lookups: stats.group_lookups(),
            max_ordinal_comparisons: stats.ordinal_comparisons(),
            max_shape_comparisons: stats.shape_comparisons(),
            max_target_case_references: stats.target_case_references(),
        }
    }

    fn exact_same_group_target_handle_limits(
        stats: GeneratedAffineResidualSameGroupTargetHandleStats,
    ) -> GeneratedAffineResidualSameGroupTargetHandleLimits {
        GeneratedAffineResidualSameGroupTargetHandleLimits {
            max_target_position_lookups: stats.target_position_lookups(),
            max_case_lookups: stats.case_lookups(),
            max_anchor_offset_lookups: stats.anchor_offset_lookups(),
            max_ordinal_comparisons: stats.ordinal_comparisons(),
        }
    }

    fn exact_same_group_target_case_limits(
        stats: GeneratedAffineResidualSameGroupTargetCaseStats,
    ) -> GeneratedAffineResidualSameGroupTargetCaseLimits {
        GeneratedAffineResidualSameGroupTargetCaseLimits {
            max_scope_comparison_bytes: stats.scope_comparison_bytes(),
            max_authority_allocation_comparisons: stats.authority_allocation_comparisons(),
            max_case_lookups: stats.case_lookups(),
            max_group_lookups: stats.group_lookups(),
            max_ordinal_comparisons: stats.ordinal_comparisons(),
            max_geometry_reference_comparisons: stats.geometry_reference_comparisons(),
        }
    }

    fn assert_initial_boolean_atoms_are_positional(
        source: GeneratedAffineInitialGlobalBooleanTerminalSourceView<'_>,
    ) {
        for (polarity, count) in [
            (
                GeneratedAffineInitialGlobalBooleanAtomPolarity::EqualZero,
            ...
            for position in 0..count {
                let atom = source.atom(polarity, position).unwrap();
                let _ = atom.locus_ordinal();
                let _ = atom.polynomial();
            }
            assert!(source.atom(polarity, count).is_none());
        }
    }

    fn assert_initial_boolean_sources_match(
        left: GeneratedAffineInitialGlobalBooleanTerminalSourceView<'_>,
        right: GeneratedAffineInitialGlobalBooleanTerminalSourceView<'_>,
    ) {
        ...
        for polarity in [
            ...
            let left_count = match polarity {
                GeneratedAffineInitialGlobalBooleanAtomPolarity::EqualZero => {
                    left.equal_zero_atom_count()
                }
                GeneratedAffineInitialGlobalBooleanAtomPolarity::NonZero => {
                    left.nonzero_atom_count()
                }
            };
            let right_count = match polarity {
                GeneratedAffineInitialGlobalBooleanAtomPolarity::EqualZero => {
                    right.equal_zero_atom_count()
                }
                GeneratedAffineInitialGlobalBooleanAtomPolarity::NonZero => {
                    right.nonzero_atom_count()
                }
            };
            assert_eq!(left_count, right_count);
            for position in 0..left_count {
                let left_atom = left.atom(polarity, position).unwrap();
                let right_atom = right.atom(polarity, position).unwrap();
                assert_eq!(left_atom.locus_ordinal(), right_atom.locus_ordinal());
                assert!(std::ptr::eq(
                    left_atom.polynomial(),
                    right_atom.polynomial()
                ));
            }
            assert!(left.atom(polarity, left_count).is_none());
            assert!(right.atom(polarity, right_count).is_none());
        }
    }

    fn assert_initial_guard_sequences_match(
        left: ResidualAffineBranchSealedGuardSourceView<'_>,
        right: ResidualAffineBranchSealedGuardSourceView<'_>,
    ) {
        assert_eq!(left.guard_count(), right.guard_count());
        assert_eq!(
            left.first_contradiction_entry_ordinal(),
            right.first_contradiction_entry_ordinal()
        );
        for position in 0..left.guard_count() {
            let left_entry = left.guard_entry(position).unwrap();
            let right_entry = right.guard_entry(position).unwrap();
            assert_eq!(
                left_entry.structural_locus_ordinal(),
                right_entry.structural_locus_ordinal()
            );
            assert!(std::ptr::eq(
                left_entry.mapped_polynomial(),
                right_entry.mapped_polynomial()
            ));
            assert_eq!(
                left_entry.composition_stats(),
                right_entry.composition_stats()
            );
            let left_class = left_entry.class();
            let right_class = right_entry.class();
            assert_eq!(
                std::mem::discriminant(&left_class),
                std::mem::discriminant(&right_class)
            );
            match (
                left_class.condition_polynomial(),
                right_class.condition_polynomial(),
            ) {
                (Some(left_condition), Some(right_condition)) => {
                    assert!(std::ptr::eq(left_condition, right_condition));
                }
                (None, None) => {}
                _ => panic!("matched guard classes expose different condition shapes"),
            }
        }
        assert!(left.guard_entry(left.guard_count()).is_none());
        assert!(right.guard_entry(right.guard_count()).is_none());
    }

    fn assert_prior_guard_sequences_match<'source>(
        left_count: usize,
        mut left_at: impl FnMut(
            usize,
        )
            -> Option<GeneratedAffineResidualBooleanPriorGuardSourceView<'source>>,
        right_count: usize,
        mut right_at: impl FnMut(
            usize,
        )
            -> Option<GeneratedAffineResidualBooleanPriorGuardSourceView<'source>>,
    ) {
        assert_eq!(left_count, right_count);
        for position in 0..left_count {
            let left = left_at(position).unwrap();
            let right = right_at(position).unwrap();
            assert_eq!(
                left.structural_locus_ordinal(),
                right.structural_locus_ordinal()
            );
            assert!(std::ptr::eq(
                left.mapped_polynomial(),
                right.mapped_polynomial()
            ));
            assert_eq!(left.composition_stats(), right.composition_stats());
            let left_class = left.class();
            let right_class = right.class();
            assert_eq!(
                std::mem::discriminant(&left_class),
                std::mem::discriminant(&right_class)
            );
            match (
                ...
                (None, None) => {}
                _ => panic!("matched prior guard classes expose different condition shapes"),
            }
        }
        assert!(left_at(left_count).is_none());
        assert!(right_at(right_count).is_none());
    }

    fn assert_prior_map_positions<'source>(
        map: &ResidualAffineIntegerMap,
        constant_count: usize,
        mut constant_at: impl FnMut(usize) -> Option<&'source Integer>,
        free_position_count: usize,
        ...
        assert_eq!(constant_count, map.ambient_arity());
        assert_eq!(free_position_count, map.free_positions().len());
        for position in 0..constant_count {
            assert_eq!(constant_at(position), map.constant(position));
        }
        assert!(constant_at(constant_count).is_none());
        for position in 0..free_position_count {
            assert_eq!(
                free_position_at(position),
                map.free_positions().get(position).copied()
            );
        }
        assert!(free_position_at(free_position_count).is_none());
    }

    fn assert_prior_actionable_terminal_and_case_match(
        terminal: GeneratedAffineResidualBooleanPriorActionableSourceView<'_>,
        case: GeneratedAffineResidualBooleanPriorActionableSourceView<'_>,
    ) {
        assert!(std::ptr::eq(terminal.affine_map(), case.affine_map()));
        assert_prior_map_positions(
            terminal.affine_map(),
            terminal.constant_count(),
            |position| terminal.constant(position),
            terminal.free_position_count(),
            |position| terminal.free_position(position),
        );
        assert_prior_guard_sequences_match(
            terminal.guard_entry_count(),
            |position| terminal.guard_entry(position),
            case.guard_entry_count(),
            |position| case.guard_entry(position),
        );
    }

    fn assert_prior_unsupported_sources_match(
        left: GeneratedAffineResidualBooleanPriorUnsupportedSourceView<'_>,
        right: GeneratedAffineResidualBooleanPriorUnsupportedSourceView<'_>,
    ) {
        for polarity in [
            GeneratedAffineResidualBooleanAtomPolarity::EqualZero,
            GeneratedAffineResidualBooleanAtomPolarity::NonZero,
        ] {
            let count = left.atom_count(polarity);
            assert_eq!(count, right.atom_count(polarity));
            for position in 0..count {
                let left_atom = left.atom(polarity, position).unwrap();
                let right_atom = right.atom(polarity, position).unwrap();
                assert_eq!(left_atom.locus_ordinal(), right_atom.locus_ordinal());
            ...
            assert!(left.atom(polarity, count).is_none());
            assert!(right.atom(polarity, count).is_none());
        }
        assert_eq!(
            left.unsupported_reason_count(),
            right.unsupported_reason_count()
        );
        for position in 0..left.unsupported_reason_count() {
            assert_eq!(
                left.unsupported_reason(position),
                right.unsupported_reason(position)
            );
        }
        assert!(
            left.unsupported_reason(left.unsupported_reason_count())
                .is_none()
        );
        assert!(
            right
                .unsupported_reason(right.unsupported_reason_count())
                .is_none()
        );
    }

    fn assert_symbolic_nu_locus_has_no_concrete_i64_witness(
        context: &ParametricCoefficientContext,
        polynomial: &ParametricPolynomial,
    ) {
        let base_names = context.base().parameter_names();
        let base_count = base_names.len();
        let nu_position = base_names
            .iter()
            .position(|name| name == "nu")
            .expect("shifted fixture retains nu in its exact base field");
        let m2_position = base_names
            .iter()
            .position(|name| name == "m2")
            .expect("shifted fixture retains m2 in its exact base field");
        let raw = polynomial.raw();
        let (invariant_term, invariant_exponents) = raw
            .exponents_iter()
            .enumerate()
            .find(|(_, exponents)| {
                exponents[nu_position] == 1
                    && exponents[m2_position] == 1
                    && exponents[base_count..]
                        .iter()
                        .all(|&exponent| exponent == 0)
            })
            .map(|(term, exponents)| (term, exponents.to_vec()))
            .expect("symbolic shifted locus contains its index-independent m2*nu monomial");
        assert_eq!(raw.coefficients[invariant_term], Integer::from(-2));
        assert_eq!(
            raw.exponents_iter()
                .filter(|exponents| exponents[..base_count] == invariant_exponents[..base_count])
                .count(),
            1,
            "no index-dependent term may cancel the m2*nu coefficient after specialization"
        );
        // Complete integer specialization changes only the private index
        // exponents.  The unique nonzero `-2*m2*nu` base monomial above is
        // therefore retained for every possible i64 assignment, proving this
        // equal-zero atom—and hence its terminal conjunction—has no concrete
        // point even though the positional symbolic terminal is authentic.
    }

    fn assert_prior_exceptional_terminal_and_case_match(
        terminal: GeneratedAffineResidualBooleanPriorExceptionalSourceView<'_>,
        case: GeneratedAffineResidualBooleanPriorExceptionalSourceView<'_>,
    ) {
        assert!(std::ptr::eq(terminal.affine_map(), case.affine_map()));
        assert_prior_map_positions(
            terminal.affine_map(),
        ...
        assert_eq!(terminal.predicate_count(), case.predicate_count());
        for position in 0..terminal.predicate_count() {
            let left = terminal.predicate(position).unwrap();
            let right = case.predicate(position).unwrap();
            assert_eq!(left.locus_ordinal(), right.locus_ordinal());
            assert_eq!(left.kind(), right.kind());
            assert!(std::ptr::eq(left.polynomial(), right.polynomial()));
        }
        assert!(terminal.predicate(terminal.predicate_count()).is_none());
        assert!(case.predicate(case.predicate_count()).is_none());
    }

    /// The retained-bit limits are charged once against the conservative
    /// pre-subtraction offset bound and again against the exact retained
    /// offset.  The public stats retain the latter, so reconstruct the exact
    /// maximum request from the authenticated private geometry for boundary
    /// tests instead of mistaking the final retained value for admission.
    fn retained_affine_limit_demands(
        inventory: &GeneratedAffineResidualCaseInventoryCertificate,
    ) -> (usize, usize) {
        let mut retained_bits = 0usize;
        let mut constant_entries = 0usize;
        let mut matrix_entries = 0usize;
        let mut offset_entries = 0usize;
        let mut maximum_bits = 0usize;
        let mut maximum_gmp = 0usize;
        for case in &inventory.cases {
            let group = &inventory.groups[case.group_ordinal];
            let anchor = &inventory.cases[group.anchor_case_ordinal];
            let constant_bits = case
                .constants
                .iter()
                .map(integer_magnitude_bits)
                .try_fold(0usize, |total, bits| {
                    checked_add("test affine constant bits", total, bits?)
                })
                .unwrap();
            let new_group_matrix_bits = if case.ordinal == group.anchor_case_ordinal {
                matrix_entries += group.compact_linear_coefficients.len();
                group
                    .compact_linear_coefficients
                    .iter()
                    .map(integer_magnitude_bits)
                    .try_fold(0usize, |total, bits| {
                        checked_add("test affine matrix bits", total, bits?)
                    })
                    .unwrap()
            } else {
                0
            };
            let offset_bit_bound = case
                .constants
                .iter()
                .zip(&anchor.constants)
                .map(|(value, anchor)| {
                    checked_add(
                        "test anchor offset bit bound",
                        integer_magnitude_bits(value)?.max(integer_magnitude_bits(anchor)?),
                        1,
                    )
                })
                .try_fold(0usize, |total, bits| {
                    checked_add("test anchor offset bit bound", total, bits?)
                })
                .unwrap();
            let exact_offset_bits = group.anchor_offsets[case.ordinal_within_group]
                .iter()
                .map(integer_magnitude_bits)
                .try_fold(0usize, |total, bits| {
                    checked_add("test exact anchor offset bits", total, bits?)
                })
                .unwrap();
            constant_entries += case.constants.len();
            offset_entries += case.constants.len();
            let integer_count = constant_entries + matrix_entries + offset_entries;
            let conservative_bits =
                retained_bits + constant_bits + new_group_matrix_bits + offset_bit_bound;
            maximum_bits = maximum_bits.max(conservative_bits);
            maximum_gmp =
                maximum_gmp.max(gmp_logical_bytes(integer_count, conservative_bits).unwrap());
            retained_bits =
                retained_bits + constant_bits + new_group_matrix_bits + exact_offset_bits;
            maximum_bits = maximum_bits.max(retained_bits);
            maximum_gmp = maximum_gmp.max(gmp_logical_bytes(integer_count, retained_bits).unwrap());
        }
        assert_eq!(
            retained_bits,
            inventory.stats().retained_affine_integer_bits()
        );
        (maximum_bits, maximum_gmp)
    }

    #[derive(Clone, Copy, Debug, PartialEq, Eq)]
    enum OuterLimitAxis {
        Terminals,
        InitialAffineChildren,
        InitialAffineProjectionAuthentications,
        SourceEmptyTerminals,
        BooleanEmptyTerminals,
        AffineEmptyTerminals,
        AffineUnsupportedTerminals,
        GuardContradictionTerminals,
        ActionableTerminals,
        PriorUnsupportedTerminals,
        PriorActionableTerminals,
        PriorExceptionalDomainTerminals,
        PriorExceptionalLeakTerminals,
        Cases,
        Groups,
        GroupCaseReferences,
        CasesPerGroup,
        SourceReferenceUnits,
        SourceReferenceBytes,
        ReadyBindingUnits,
        ReadyBindingBytes,
        AmbientArity,
        FreePositionReferences,
        CompactMatrixEntriesInspected,
        RetainedCompactMatrixEntries,
        ConstantEntries,
        AffineIntegerBits,
        AffineIntegerBitsInspected,
        RetainedAffineIntegerBits,
        GroupComparisons,
        GroupComparisonEntriesInspected,
        GroupComparisonIntegerBitWork,
        AnchorOffsetEntries,
        AnchorOffsetIntegerBitWork,
        AnchorOffsetIntegerBits,
        RetainedGmpLogicalBytes,
        ChildRetainedOwnedLogicalBytes,
        ChildCompilationOwnedLogicalPeak,
        RetainedOwnedLogicalBytes,
        TemporaryOwnedLogicalBytes,
        CompilationOwnedLogicalPeak,
        ReplayOwnedLogicalPeak,
        PayloadComparisonUnits,
        PayloadComparisonBytes,
        PayloadComparisonIntegerBits,
        RecursiveChildComparisonUnits,
        RecursiveChildComparisonBytes,
        RecursiveChildComparisonIntegerBits,
        ReadyBindingPairUnits,
        ReadyBindingPairBytes,
    }

    #[derive(Clone, Copy, Debug, PartialEq, Eq)]
    enum OuterLimitFailurePhase {
        BeforeOuterReserve,
        BeforeCurrentReadyConsumption,
        Unpinned,
    }

    impl OuterLimitAxis {
        fn demand(self, inventory: &GeneratedAffineResidualCaseInventoryCertificate) -> usize {
            let stats = inventory.stats();
            match self {
                Self::Terminals => stats.terminals(),
                Self::InitialAffineChildren => stats.initial_affine_children(),
                Self::InitialAffineProjectionAuthentications => {
                    stats.initial_affine_projection_authentications()
                }
                Self::SourceEmptyTerminals => stats.source_empty_terminals(),
                Self::BooleanEmptyTerminals => stats.boolean_empty_terminals(),
                Self::AffineEmptyTerminals => stats.affine_empty_terminals(),
                Self::AffineUnsupportedTerminals => stats.affine_unsupported_terminals(),
                Self::GuardContradictionTerminals => stats.guard_contradiction_terminals(),
                Self::ActionableTerminals => stats.actionable_terminals(),
                Self::PriorUnsupportedTerminals => stats.prior_unsupported_terminals(),
                Self::PriorActionableTerminals => stats.prior_actionable_terminals(),
                Self::PriorExceptionalDomainTerminals => stats.prior_exceptional_domain_terminals(),
                Self::PriorExceptionalLeakTerminals => stats.prior_exceptional_leak_terminals(),
                Self::Cases => stats.cases(),
                Self::Groups => stats.groups(),
                Self::GroupCaseReferences => stats.group_case_references(),
                Self::CasesPerGroup => stats.maximum_cases_per_group(),
                Self::SourceReferenceUnits => stats.source_reference_units(),
                Self::SourceReferenceBytes => stats.source_reference_bytes(),
                Self::ReadyBindingUnits => stats.ready_binding_units(),
                Self::ReadyBindingBytes => stats.ready_binding_bytes(),
                Self::AmbientArity => stats.maximum_ambient_arity(),
                Self::FreePositionReferences => stats
                    .free_position_references()
                    .max(stats.retained_free_position_references()),
                Self::CompactMatrixEntriesInspected => stats.compact_matrix_entries_inspected(),
                Self::RetainedCompactMatrixEntries => stats.retained_compact_matrix_entries(),
                Self::ConstantEntries => stats.constant_entries(),
                Self::AffineIntegerBits => stats.maximum_affine_integer_bits(),
                Self::AffineIntegerBitsInspected => stats.affine_integer_bits_inspected(),
                Self::RetainedAffineIntegerBits => retained_affine_limit_demands(inventory).0,
                Self::GroupComparisons => stats.group_comparisons(),
                Self::GroupComparisonEntriesInspected => stats.group_comparison_entries_inspected(),
                Self::GroupComparisonIntegerBitWork => stats.group_comparison_integer_bit_work(),
                Self::AnchorOffsetEntries => stats.anchor_offset_entries(),
                Self::AnchorOffsetIntegerBitWork => stats.anchor_offset_integer_bit_work(),
                Self::AnchorOffsetIntegerBits => stats.anchor_offset_integer_bits(),
                Self::RetainedGmpLogicalBytes => retained_affine_limit_demands(inventory).1,
                Self::ChildRetainedOwnedLogicalBytes => {
                    stats.child_retained_owned_logical_bytes_admission_demand()
                }
                Self::ChildCompilationOwnedLogicalPeak => {
                    stats.child_compilation_owned_logical_peak_admission_demand()
                }
                Self::RetainedOwnedLogicalBytes => {
                    stats.retained_owned_logical_bytes_admission_demand()
                }
                Self::TemporaryOwnedLogicalBytes => stats.temporary_owned_logical_bytes(),
                Self::CompilationOwnedLogicalPeak => {
                    stats.compilation_owned_logical_peak_admission_demand()
                }
                Self::ReplayOwnedLogicalPeak => stats.replay_owned_logical_peak(),
                Self::PayloadComparisonUnits => stats.payload_comparison_units(),
                Self::PayloadComparisonBytes => stats.payload_comparison_bytes(),
                Self::PayloadComparisonIntegerBits => stats.payload_comparison_integer_bits(),
                Self::RecursiveChildComparisonUnits => {
                    stats.recursive_child_comparison_units_admission_demand()
                }
                Self::RecursiveChildComparisonBytes => {
                    stats.recursive_child_comparison_bytes_admission_demand()
                }
                Self::RecursiveChildComparisonIntegerBits => {
                    stats.recursive_child_comparison_integer_bits_admission_demand()
                }
                Self::ReadyBindingPairUnits => stats.ready_binding_pair_units(),
                Self::ReadyBindingPairBytes => stats.ready_binding_pair_bytes(),
            }
        }

        fn set(self, limits: &mut GeneratedAffineResidualCaseInventoryLimits, value: usize) {
            match self {
                Self::Terminals => limits.max_terminals = value,
                Self::InitialAffineChildren => limits.max_initial_affine_children = value,
                Self::InitialAffineProjectionAuthentications => {
                    limits.max_initial_affine_projection_authentications = value;
                }
                Self::SourceEmptyTerminals => limits.max_source_empty_terminals = value,
                Self::BooleanEmptyTerminals => limits.max_boolean_empty_terminals = value,
                Self::AffineEmptyTerminals => limits.max_affine_empty_terminals = value,
                Self::AffineUnsupportedTerminals => {
                    limits.max_affine_unsupported_terminals = value;
                }
                Self::GuardContradictionTerminals => {
                    limits.max_guard_contradiction_terminals = value;
                }
                Self::ActionableTerminals => limits.max_actionable_terminals = value,
                Self::PriorUnsupportedTerminals => {
                    limits.max_prior_unsupported_terminals = value;
                }
                Self::PriorActionableTerminals => limits.max_prior_actionable_terminals = value,
                Self::PriorExceptionalDomainTerminals => {
                    limits.max_prior_exceptional_domain_terminals = value;
                }
                Self::PriorExceptionalLeakTerminals => {
                    limits.max_prior_exceptional_leak_terminals = value;
                }
                Self::Cases => limits.max_cases = value,
                Self::Groups => limits.max_groups = value,
                Self::GroupCaseReferences => limits.max_group_case_references = value,
                Self::CasesPerGroup => limits.max_cases_per_group = value,
                Self::SourceReferenceUnits => limits.max_source_reference_units = value,
                Self::SourceReferenceBytes => limits.max_source_reference_bytes = value,
                Self::ReadyBindingUnits => limits.max_ready_binding_units = value,
                Self::ReadyBindingBytes => limits.max_ready_binding_bytes = value,
                Self::AmbientArity => limits.max_ambient_arity = value,
                Self::FreePositionReferences => limits.max_free_position_references = value,
                Self::CompactMatrixEntriesInspected => {
                    limits.max_compact_matrix_entries_inspected = value;
                }
                Self::RetainedCompactMatrixEntries => {
                    limits.max_retained_compact_matrix_entries = value;
                }
                Self::ConstantEntries => limits.max_constant_entries = value,
                Self::AffineIntegerBits => limits.max_affine_integer_bits = value,
                Self::AffineIntegerBitsInspected => {
                    limits.max_affine_integer_bits_inspected = value;
                }
                Self::RetainedAffineIntegerBits => {
                    limits.max_retained_affine_integer_bits = value;
                }
                Self::GroupComparisons => limits.max_group_comparisons = value,
                Self::GroupComparisonEntriesInspected => {
                    limits.max_group_comparison_entries_inspected = value;
                }
                Self::GroupComparisonIntegerBitWork => {
                    limits.max_group_comparison_integer_bit_work = value;
                }
                Self::AnchorOffsetEntries => limits.max_anchor_offset_entries = value,
                Self::AnchorOffsetIntegerBitWork => {
                    limits.max_anchor_offset_integer_bit_work = value;
                }
                Self::AnchorOffsetIntegerBits => limits.max_anchor_offset_integer_bits = value,
                Self::RetainedGmpLogicalBytes => limits.max_retained_gmp_logical_bytes = value,
                Self::ChildRetainedOwnedLogicalBytes => {
                    limits.max_child_retained_owned_logical_bytes = value;
                }
                Self::ChildCompilationOwnedLogicalPeak => {
                    limits.max_child_compilation_owned_logical_peak = value;
                }
                Self::RetainedOwnedLogicalBytes => {
                    limits.max_retained_owned_logical_bytes = value;
                }
                Self::TemporaryOwnedLogicalBytes => {
                    limits.max_temporary_owned_logical_bytes = value;
                }
                Self::CompilationOwnedLogicalPeak => {
                    limits.max_compilation_owned_logical_peak = value;
                }
                Self::ReplayOwnedLogicalPeak => limits.max_replay_owned_logical_peak = value,
                Self::PayloadComparisonUnits => limits.max_payload_comparison_units = value,
                Self::PayloadComparisonBytes => limits.max_payload_comparison_bytes = value,
                Self::PayloadComparisonIntegerBits => {
                    limits.max_payload_comparison_integer_bits = value;
                }
                Self::RecursiveChildComparisonUnits => {
                    limits.max_recursive_child_comparison_units = value;
                }
                Self::RecursiveChildComparisonBytes => {
                    limits.max_recursive_child_comparison_bytes = value;
                }
                Self::RecursiveChildComparisonIntegerBits => {
                    limits.max_recursive_child_comparison_integer_bits = value;
                }
                Self::ReadyBindingPairUnits => limits.max_ready_binding_pair_units = value,
                Self::ReadyBindingPairBytes => limits.max_ready_binding_pair_bytes = value,
            }
        }

        const fn resource(self) -> &'static str {
            match self {
                Self::Terminals => "inventory terminals",
                Self::InitialAffineChildren => "initial affine children",
                Self::InitialAffineProjectionAuthentications => {
                    "initial affine projection authentications"
                }
                Self::SourceEmptyTerminals => "source-empty terminals",
                Self::BooleanEmptyTerminals => "Boolean-empty terminals",
                Self::AffineEmptyTerminals => "affine-empty terminals",
                Self::AffineUnsupportedTerminals => "affine-unsupported terminals",
                Self::GuardContradictionTerminals => "guard-contradiction terminals",
                Self::ActionableTerminals => "actionable terminals",
                Self::PriorUnsupportedTerminals => "prior unsupported terminals",
                Self::PriorActionableTerminals => "prior actionable terminals",
                Self::PriorExceptionalDomainTerminals => "prior exceptional-domain terminals",
                Self::PriorExceptionalLeakTerminals => "prior exceptional-leak terminals",
                Self::Cases => "actionable cases",
                Self::Groups => "affine geometry groups",
                Self::GroupCaseReferences => "group case references",
                Self::CasesPerGroup => "cases in one affine geometry group",
                Self::SourceReferenceUnits => "source reference units",
                Self::SourceReferenceBytes => "source reference bytes",
                Self::ReadyBindingUnits => "ready binding units",
                Self::ReadyBindingBytes => "ready binding bytes",
                Self::AmbientArity => "ambient affine arity",
                Self::FreePositionReferences => "free-position references",
                Self::CompactMatrixEntriesInspected => "compact affine matrix entries inspected",
                Self::RetainedCompactMatrixEntries => "retained compact affine matrix entries",
                Self::ConstantEntries => "affine constant entries",
                Self::AffineIntegerBits => "individual affine integer bits",
                Self::AffineIntegerBitsInspected => "affine integer bits inspected",
                Self::RetainedAffineIntegerBits => "retained affine integer bits",
                Self::GroupComparisons => "affine geometry group comparisons",
                Self::GroupComparisonEntriesInspected => {
                    "affine geometry comparison entries inspected"
                }
                Self::GroupComparisonIntegerBitWork => {
                ...
                Self::AnchorOffsetEntries => "anchor offset entries",
                Self::AnchorOffsetIntegerBitWork => "anchor offset integer-bit work",
                Self::AnchorOffsetIntegerBits => "anchor offset integer bits",
                Self::RetainedGmpLogicalBytes => "retained GMP logical bytes",
                Self::ChildRetainedOwnedLogicalBytes => "child retained owned logical bytes",
                Self::ChildCompilationOwnedLogicalPeak => "child compilation owned logical peak",
                Self::RetainedOwnedLogicalBytes => "retained owned logical bytes",
                Self::TemporaryOwnedLogicalBytes => "temporary owned logical bytes",
                Self::CompilationOwnedLogicalPeak => "compilation owned logical peak",
                Self::ReplayOwnedLogicalPeak => "replay owned logical peak",
                Self::PayloadComparisonUnits => "payload comparison units",
                Self::PayloadComparisonBytes => "payload comparison bytes",
                Self::PayloadComparisonIntegerBits => "payload comparison integer bits",
                Self::RecursiveChildComparisonUnits => "recursive child comparison units",
                Self::RecursiveChildComparisonBytes => "recursive child comparison bytes",
                Self::RecursiveChildComparisonIntegerBits => {
                    "recursive child comparison integer bits"
                }
                Self::ReadyBindingPairUnits => "ready binding pair units",
                Self::ReadyBindingPairBytes => "ready binding pair bytes",
            }
        }

        const fn failure_phase(self) -> OuterLimitFailurePhase {
            match self {
                Self::Terminals
                | Self::InitialAffineChildren
                | Self::SourceEmptyTerminals
                | Self::BooleanEmptyTerminals
                | Self::PriorUnsupportedTerminals
                | Self::PriorActionableTerminals
                | Self::PriorExceptionalDomainTerminals
                | Self::PriorExceptionalLeakTerminals
                | Self::ChildRetainedOwnedLogicalBytes
                | Self::ChildCompilationOwnedLogicalPeak
                ...
                | Self::RecursiveChildComparisonBytes
                | Self::RecursiveChildComparisonIntegerBits => {
                    OuterLimitFailurePhase::BeforeOuterReserve
                }
                Self::ReadyBindingUnits
                | Self::ReadyBindingBytes
                | Self::ReadyBindingPairUnits
                | Self::ReadyBindingPairBytes => {
                    OuterLimitFailurePhase::BeforeCurrentReadyConsumption
                }
                _ => OuterLimitFailurePhase::Unpinned,
            }
        }
    }

    const INITIAL_COUNT_LIMIT_AXES: &[OuterLimitAxis] = &[
        OuterLimitAxis::Terminals,
        OuterLimitAxis::InitialAffineChildren,
        OuterLimitAxis::InitialAffineProjectionAuthentications,
        OuterLimitAxis::BooleanEmptyTerminals,
        OuterLimitAxis::ActionableTerminals,
        OuterLimitAxis::Cases,
        OuterLimitAxis::Groups,
        OuterLimitAxis::GroupCaseReferences,
        OuterLimitAxis::CasesPerGroup,
    ];
    const SOURCE_BINDING_LIMIT_AXES: &[OuterLimitAxis] = &[
        OuterLimitAxis::SourceReferenceUnits,
        OuterLimitAxis::SourceReferenceBytes,
        OuterLimitAxis::ReadyBindingUnits,
        OuterLimitAxis::ReadyBindingBytes,
        OuterLimitAxis::ReadyBindingPairUnits,
        OuterLimitAxis::ReadyBindingPairBytes,
    ];
    const GEOMETRY_LIMIT_AXES: &[OuterLimitAxis] = &[
        OuterLimitAxis::AmbientArity,
        OuterLimitAxis::FreePositionReferences,
        OuterLimitAxis::CompactMatrixEntriesInspected,
        OuterLimitAxis::RetainedCompactMatrixEntries,
        OuterLimitAxis::ConstantEntries,
        OuterLimitAxis::AffineIntegerBits,
        OuterLimitAxis::AffineIntegerBitsInspected,
        OuterLimitAxis::RetainedAffineIntegerBits,
        OuterLimitAxis::GroupComparisons,
        OuterLimitAxis::GroupComparisonEntriesInspected,
        OuterLimitAxis::GroupComparisonIntegerBitWork,
        OuterLimitAxis::AnchorOffsetEntries,
        OuterLimitAxis::AnchorOffsetIntegerBitWork,
        OuterLimitAxis::AnchorOffsetIntegerBits,
        OuterLimitAxis::RetainedGmpLogicalBytes,
    ];
    const MEMORY_LIMIT_AXES: &[OuterLimitAxis] = &[
        OuterLimitAxis::ChildRetainedOwnedLogicalBytes,
        OuterLimitAxis::ChildCompilationOwnedLogicalPeak,
        OuterLimitAxis::RetainedOwnedLogicalBytes,
        OuterLimitAxis::TemporaryOwnedLogicalBytes,
        OuterLimitAxis::CompilationOwnedLogicalPeak,
        OuterLimitAxis::ReplayOwnedLogicalPeak,
    ];
    const PAYLOAD_LIMIT_AXES: &[OuterLimitAxis] = &[
        OuterLimitAxis::PayloadComparisonUnits,
        OuterLimitAxis::PayloadComparisonBytes,
        OuterLimitAxis::PayloadComparisonIntegerBits,
        OuterLimitAxis::RecursiveChildComparisonUnits,
        OuterLimitAxis::RecursiveChildComparisonBytes,
        OuterLimitAxis::RecursiveChildComparisonIntegerBits,
    ];
    const NATURAL_VARIANT_LIMIT_AXES: &[OuterLimitAxis] = &[
        OuterLimitAxis::AffineUnsupportedTerminals,
        OuterLimitAxis::PriorUnsupportedTerminals,
        OuterLimitAxis::PriorActionableTerminals,
        OuterLimitAxis::PriorExceptionalDomainTerminals,
        OuterLimitAxis::PriorExceptionalLeakTerminals,
    ];
    const ALL_OUTER_LIMIT_AXES: &[OuterLimitAxis] = &[
        OuterLimitAxis::Terminals,
        OuterLimitAxis::InitialAffineChildren,
        OuterLimitAxis::InitialAffineProjectionAuthentications,
        OuterLimitAxis::SourceEmptyTerminals,
        OuterLimitAxis::BooleanEmptyTerminals,
        OuterLimitAxis::AffineEmptyTerminals,
        OuterLimitAxis::AffineUnsupportedTerminals,
        OuterLimitAxis::GuardContradictionTerminals,
        OuterLimitAxis::ActionableTerminals,
        OuterLimitAxis::PriorUnsupportedTerminals,
        OuterLimitAxis::PriorActionableTerminals,
        OuterLimitAxis::PriorExceptionalDomainTerminals,
    ...
    fn exercise_positive_outer_limit_table(
        family: &IntegralFamily,
        context: &ParametricCoefficientContext,
        source: &Arc<GeneratedAffineResidualBooleanCoverCertificate>,
        baseline: &GeneratedAffineResidualCaseInventoryCertificate,
        axes: &[OuterLimitAxis],
    ) {
        let demands = axes
            .iter()
            .copied()
            .map(|axis| {
                let demand = axis.demand(baseline);
                assert!(demand > 0, "{axis:?} must be a positive natural demand");
                (axis, demand)
            })
            .collect::<Vec<_>>();
        let mut exact_limits = baseline.limits();
        for &(axis, demand) in &demands {
            axis.set(&mut exact_limits, demand);
        }
        let exact = GeneratedAffineResidualCaseInventoryCompiler::compile(
            family,
            context,
            Arc::clone(source),
            exact_limits,
        ...
        .unwrap();
        for &(axis, demand) in &demands {
            assert_eq!(axis.demand(&exact), demand, "exact {axis:?} demand moved");
        }
        exact.replay(family, context).unwrap();

        for (axis, demand) in demands {
            let mut limits = baseline.limits();
            axis.set(&mut limits, demand - 1);
            reset_inventory_ready_consumptions_for_test();
            reset_inventory_compilations_for_test();
            reset_inventory_outer_reserves_for_test();
            let error = GeneratedAffineResidualCaseInventoryCompiler::compile(
                family,
                context,
                Arc::clone(source),
                limits,
            ...
            .err()
            .expect("one-below natural demand must reject");
            assert!(
                matches!(
                    error,
                    GeneratedAffineResidualCaseInventoryError::ResourceLimit {
                        ...
                        requested,
                        limit,
                    } if resource == axis.resource()
                        && requested == demand
                        && limit == demand - 1
                ),
                "unexpected {axis:?} one-below error: {error:?}"
            );
            assert_eq!(inventory_compilations_for_test(), 1);
            match axis.failure_phase() {
                OuterLimitFailurePhase::BeforeOuterReserve => {
                    assert_eq!(inventory_outer_reserves_for_test(), 0, "{axis:?}");
                    assert_eq!(inventory_ready_consumptions_for_test(), 0, "{axis:?}");
                }
                OuterLimitFailurePhase::BeforeCurrentReadyConsumption => {
                    assert_eq!(inventory_outer_reserves_for_test(), 1, "{axis:?}");
                    assert!(
                        inventory_ready_consumptions_for_test()
                            < baseline.stats().initial_affine_children(),
                        "{axis:?} must reject before consuming the Ready record that exceeds it"
                    );
                }
                OuterLimitFailurePhase::Unpinned => {}
            }
        }
    }
    ...
    #[test]
    fn one_loop_inventory_compiles_authenticates_and_replays() {
        assert_send_sync::<GeneratedAffineResidualCaseInventoryCertificate>();
        let (family, context, boolean, inventory) =
            one_loop_fixture("v2-affine-inventory-one-loop-private");
        assert_eq!(
            inventory.schema(),
            GENERATED_AFFINE_RESIDUAL_CASE_INVENTORY_V2_SCHEMA
        );
        assert!(Arc::ptr_eq(&inventory.source_boolean_cover, &boolean));
        assert_eq!(inventory.terminal_count(), boolean.terminal_count());
        assert_eq!(
            inventory.stats().initial_affine_children(),
            boolean.stats().ready_for_affine_recognition_terminals()
        );
        for ordinal in 0..inventory.terminal_count() {
            let view = inventory
                .authenticated_terminal_view(&context, ordinal)
            ...
            assert_eq!(view.locator().boolean_record_ordinal(), ordinal);
        }
        for ordinal in 0..inventory.case_count() {
            let view = inventory
                .authenticated_case_view(&context, ordinal)
            ...
            assert_eq!(
                view.constants().len(),
                view.source().affine_map().ambient_arity()
            );
        }
        for ordinal in 0..inventory.group_count() {
            let view = inventory
                .authenticated_group_view(&context, ordinal)
                .unwrap();
            assert_eq!(view.ordinal(), ordinal);
            assert!(!view.case_ordinals().is_empty());
        }
    ...
    #[test]
    fn source_neutral_point_chain_and_case_authority_bind_natural_s2() {
        let (family, context, _, inventory) =
            two_loop_fixture("v2-affine-inventory-point-authority-private");
        let inventory = Arc::new(inventory);
        let indices = [0, 1, 2];

        let boolean = inventory
            .source_boolean_cover
            .classification_for_indices(
                &family,
                &context,
        ...
        assert!(matches!(
            boolean.disposition(),
            GeneratedAffineResidualBooleanPointDisposition::Terminal {
                outcome: GeneratedAffineResidualBooleanTerminalOutcome::ReadyForAffineRecognition,
                ..
            }
        ));
        ...
        let exact = inventory
            ...
            .unwrap();
        let GeneratedAffineResidualInventoryPointDisposition::Actionable {
            terminal_ordinal,
            case_ordinal,
        } = exact.disposition()
        else {
            panic!("natural 011 point must resolve to one actionable case");
        };
        assert!(exact.stats().boolean().terminal_record_scans() > 0);
        assert!(exact.stats().boolean().initial_boolean().atom_evaluations() > 0);
        assert!(exact.stats().case_scans() > 0);
        assert!(exact.stats().affine_map().is_some());
        assert_inventory_point_exact_and_every_positive_one_below(
            &family,
            &context,
            inventory.as_ref(),
            &indices,
        ...
        let exact_case = inventory
            .authenticated_case_view(&context, case_ordinal)
            .unwrap();
        let constrained_position = (0..indices.len())
            .find(|position| {
                !exact_case
                    .source()
                    .affine_map()
            ...
            .expect("natural actionable S2 case has a constrained coordinate");
        let mut non_fixed = indices;
        non_fixed[constrained_position] += 1;
        assert!(matches!(
            inventory_case_fixed_point_stats(
                exact_case,
                &non_fixed,
                ResidualAffineIntegerMapPointLimits::default(),
            ),
            Err(GeneratedAffineResidualInventoryPointError::AffineMapDoesNotFixPoint)
        ));

        let mut authority_one_below = GeneratedAffineResidualCaseAuthorityLimits::default();
        authority_one_below.max_inventory_replays = 0;
        assert!(matches!(
            GeneratedAffineResidualCaseAuthority::try_new(
                &family,
                &context,
            ...
            Err(GeneratedAffineResidualCaseAuthorityError::ResourceLimit {
                resource: "inventory replays",
                requested: 1,
                limit: 0,
            })
        ));

        let authority = GeneratedAffineResidualCaseAuthority::try_new(
            &family,
            &context,
            Arc::clone(&inventory),
            case_ordinal,
        ...
        .unwrap();
        let group_authentication = assert_inventory_group_exact_and_every_positive_one_below(
            &context,
            inventory.as_ref(),
            authority.group_ordinal(),
        );
        assert_eq!(authority.stats().authentication(), group_authentication);
        assert_case_authority_exact_and_every_positive_replay_one_below(
            &family,
            &context,
            &inventory,
            case_ordinal,
        ...
        assert!(authority.same_inventory_allocation(&inventory));
        assert_eq!(authority.case_ordinal(), case_ordinal);
        assert_eq!(authority.arity(), indices.len());
        assert_eq!(
            authority.sector(),
            &SectorMask::try_from_bit_string("011").unwrap()
        );
        ...
        let authenticated_group = authority.authenticated_group_view(&context).unwrap();
        assert_eq!(authenticated_group.ordinal(), authority.group_ordinal());
        assert_eq!(authority.stats().inventory_replays(), 1);
        assert_eq!(
            authority.stats().case_authentications(),
            authenticated_group.case_ordinals().len()
        );
        assert_eq!(authority.stats().group_authentications(), 1);
        assert!(authority.stats().scope_comparison_bytes() > 0);
        authority.replay(&family, &context).unwrap();

        let wrong_family =
            equal_mass_two_loop_family("v2-affine-inventory-point-authority-wrong-family-private");
        assert!(matches!(
            GeneratedAffineResidualCaseAuthority::try_new(
                &wrong_family,
                &context,
            ...
            Err(GeneratedAffineResidualCaseAuthorityError::WrongFamily)
        ));
        let wrong_scope = ParametricCoefficientContext::try_new(
            family.coefficient_context(),
            "v2-affine-inventory-point-authority-wrong-scope-private",
            context.index_count(),
        )
        .unwrap();
        assert!(matches!(
            ...
            Err(GeneratedAffineResidualCaseAuthorityError::WrongContext)
        ));
        let wrong_arity = ParametricCoefficientContext::try_new(
            family.coefficient_context(),
            "v2-affine-inventory-point-authority-wrong-arity-private",
        ...
        assert!(matches!(
            ...
            Err(GeneratedAffineResidualCaseAuthorityError::WrongArity)
        ));
        let mut invalid_ordinal_limits = GeneratedAffineResidualCaseAuthorityLimits::default();
        invalid_ordinal_limits.max_inventory_replays = 0;
        assert!(matches!(
            GeneratedAffineResidualCaseAuthority::try_new(
                &family,
                &context,
                ...
                invalid_ordinal_limits,
            ),
            Err(GeneratedAffineResidualCaseAuthorityError::CaseOutOfRange)
        ));

        assert!(inventory.source_row_count() > 0);
        let source_row_ordinal = (0..inventory.source_row_count())
            .max_by_key(|&ordinal| {
                let relation = inventory.source_row(ordinal).unwrap();
                (
                    relation.guarded_nonzero_conditions().len(),
                    relation.terms().len(),
                )
        ...
        assert_eq!(row.relation().family_fingerprint(), family.fingerprint());
        assert_eq!(row.relation().context_fingerprint(), context.fingerprint());
        assert_eq!(row.stats().source_rows(), inventory.source_row_count());
        assert_source_row_exact_and_every_positive_one_below(
            &authority,
            &family,
            &context,
            source_row_ordinal,
        ...
        assert!(matches!(
            inventory
                ...
                .unwrap()
                .disposition(),
            GeneratedAffineResidualInventoryPointDisposition::Excluded
        ));

        let mut one_below = GeneratedAffineResidualInventoryPointLimits::default();
        one_below.boolean.max_terminal_record_scans =
            exact.stats().boolean().terminal_record_scans() - 1;
        assert!(matches!(
            inventory.classification_for_indices(&family, &context, &indices, one_below),
            Err(GeneratedAffineResidualInventoryPointError::Boolean(
                GeneratedAffineResidualBooleanPointError::ResourceLimit { .. }
            ))
        ));
        let mut atom_one_below = GeneratedAffineResidualInventoryPointLimits::default();
        atom_one_below.boolean.initial_boolean.max_atom_evaluations =
            exact.stats().boolean().initial_boolean().atom_evaluations() - 1;
        assert!(matches!(
            inventory.classification_for_indices(&family, &context, &indices, atom_one_below),
            Err(GeneratedAffineResidualInventoryPointError::Boolean(
                GeneratedAffineResidualBooleanPointError::InitialBoolean(
                    crate::generated_affine_residual_source_authority::GeneratedAffineInitialGlobalBooleanPointError::ResourceLimit { .. }
                )
            ))
        ));

        let debug = format!("{authority:?} {row:?}");
        assert!(debug.contains("<redacted>"));
        assert!(!debug.contains("m2"));
    }

    #[test]
    fn same_group_target_handles_are_bounded_borrowed_and_allocation_sealed() {
        let (family, context, boolean, inventory) =
            two_loop_fixture("v2-affine-inventory-same-group-target-private");
        let inventory = Arc::new(inventory);
        let retained_group = inventory
            .groups
            .iter()
            .max_by_key(|group| group.case_ordinals.len())
            .expect("natural S2 retains actionable geometry groups");
        assert!(retained_group.case_ordinals.len() > 1);
        let source_case_ordinal = retained_group.anchor_case_ordinal;
        let authority = Arc::new(
            GeneratedAffineResidualCaseAuthority::try_new(
                &family,
                &context,
        ...
        assert_eq!(authority.case_ordinal(), independent.case_ordinal());
        assert_eq!(authority.group_ordinal(), independent.group_ordinal());
        assert_eq!(authority.source_row_count(), inventory.source_row_count());
        ...
        assert_eq!(baseline.source_case_ordinal(), source_case_ordinal);
        assert_eq!(baseline.group_ordinal(), retained_group.ordinal);
        assert_eq!(baseline.len(), retained_group.case_ordinals.len());
        let collection_stats = baseline.stats();
        let exact_collection_limits = exact_same_group_target_cases_limits(collection_stats);
        let exact_collection = authority
            .same_group_target_cases(&family, &context, exact_collection_limits)
            .unwrap();
        assert_eq!(exact_collection.stats(), collection_stats);

        macro_rules! collection_one_below {
            ($limit:ident, $stat:ident, $resource:literal) => {
                if collection_stats.$stat() > 0 {
                    let mut limits = exact_collection_limits;
                    limits.$limit = collection_stats.$stat() - 1;
                    assert!(matches!(
                        authority.same_group_target_cases(&family, &context, limits),
                        Err(GeneratedAffineResidualCaseAuthorityError::ResourceLimit {
                            resource: $resource,
                            requested,
                            limit,
                        }) if requested == collection_stats.$stat()
                            && limit == collection_stats.$stat() - 1
                    ));
                }
            };
        }
        collection_one_below!(
            max_scope_comparison_bytes,
            scope_comparison_bytes,
            "scope comparison bytes"
        );
        collection_one_below!(max_case_lookups, case_lookups, "same-group case lookups");
        collection_one_below!(max_group_lookups, group_lookups, "same-group group lookups");
        collection_one_below!(
            max_ordinal_comparisons,
            ordinal_comparisons,
            "same-group ordinal comparisons"
        );
        collection_one_below!(
            max_shape_comparisons,
            shape_comparisons,
            "same-group shape comparisons"
        );
        collection_one_below!(
            max_target_case_references,
            target_case_references,
            "same-group target case references"
        );

        let target_position = baseline
            .group
            .case_ordinals()
            .iter()
            .position(|&ordinal| ordinal != source_case_ordinal)
            .expect("largest natural S2 group has another target");
        let handle = baseline
            .target(
                target_position,
            ...
            .unwrap();
        assert_ne!(handle.case_ordinal(), source_case_ordinal);
        assert_eq!(handle.group_ordinal(), authority.group_ordinal());
        assert_eq!(handle.ordinal_within_group(), target_position);
        assert_eq!(handle.family_fingerprint(), family.fingerprint());
        assert_eq!(handle.context_fingerprint(), context.fingerprint());
        assert_eq!(handle.ambient_arity(), authority.arity());
        assert_eq!(handle.constants().len(), authority.arity());
        assert_eq!(handle.anchor_offset().len(), authority.arity());
        assert_eq!(
            handle.compact_linear_coefficients().len(),
            handle.ambient_arity() * handle.free_positions().len()
        );
        let handle_stats = handle.stats();
        let exact_handle_limits = exact_same_group_target_handle_limits(handle_stats);
        let exact_handle = baseline
            .target(target_position, exact_handle_limits)
            .unwrap();
        assert_eq!(exact_handle.stats(), handle_stats);

        macro_rules! handle_one_below {
            ($limit:ident, $stat:ident, $resource:literal) => {
                if handle_stats.$stat() > 0 {
                    let mut limits = exact_handle_limits;
                    limits.$limit = handle_stats.$stat() - 1;
                    assert!(matches!(
                        baseline.target(target_position, limits),
                        Err(GeneratedAffineResidualCaseAuthorityError::ResourceLimit {
                            resource: $resource,
                            requested,
                            limit,
                        }) if requested == handle_stats.$stat()
                            && limit == handle_stats.$stat() - 1
                    ));
                }
            };
        }
        handle_one_below!(
            max_target_position_lookups,
            target_position_lookups,
            "same-group target position lookups"
        );
        handle_one_below!(
        ...
        assert!(matches!(
            ...
            Err(GeneratedAffineResidualCaseAuthorityError::TargetPositionOutOfRange)
        ));
        ...
        assert_eq!(target.ordinal(), handle.case_ordinal());
        assert_eq!(target.group_ordinal(), handle.group_ordinal());
        assert_eq!(target.ordinal_within_group(), handle.ordinal_within_group());
        assert!(std::ptr::eq(target.constants(), handle.constants()));
        assert_eq!(
            target.source().affine_map().free_positions(),
            handle.free_positions()
        );
        for row in 0..handle.ambient_arity() {
            for (compact_column, &column) in handle.free_positions().iter().enumerate() {
                assert_eq!(
                    target.source().affine_map().linear_coefficient(row, column),
                    handle
                        .compact_linear_coefficients()
                        .get(row * handle.free_positions().len() + compact_column)
                );
            }
        }
        let target_stats = resolved.stats();
        let exact_target_limits = exact_same_group_target_case_limits(target_stats);
        let exact_target = authority
            .authenticated_same_group_target_case_view(
                &family,
                &context,
                handle,
            ...
            .unwrap();
        assert_eq!(exact_target.stats(), target_stats);

        macro_rules! target_one_below {
            ($limit:ident, $stat:ident, $resource:literal) => {
                if target_stats.$stat() > 0 {
                    let mut limits = exact_target_limits;
                    limits.$limit = target_stats.$stat() - 1;
                    assert!(matches!(
                        authority.authenticated_same_group_target_case_view(
                            &family,
                            &context,
                            handle,
                            limits,
                        ...
                        Err(GeneratedAffineResidualCaseAuthorityError::ResourceLimit {
                            ...
                            requested,
                            limit,
                        }) if requested == target_stats.$stat()
                            && limit == target_stats.$stat() - 1
                    ));
                }
            };
        }
        ...
        target_one_below!(
            max_authority_allocation_comparisons,
            authority_allocation_comparisons,
            "target authority allocation comparisons"
        );
        target_one_below!(max_case_lookups, case_lookups, "target case lookups");
        target_one_below!(max_group_lookups, group_lookups, "target group lookups");
        target_one_below!(
            max_ordinal_comparisons,
            ordinal_comparisons,
            "target ordinal comparisons"
        ...
        target_one_below!(
            max_geometry_reference_comparisons,
            geometry_reference_comparisons,
            "target geometry reference comparisons"
        );

        assert!(matches!(
            independent.authenticated_same_group_target_case_view(
                &family,
                &context,
                handle,
                GeneratedAffineResidualSameGroupTargetCaseLimits::default(),
            ),
            Err(GeneratedAffineResidualCaseAuthorityError::WrongAuthorityAllocation)
        ));
        let mut wrong_case = handle;
        wrong_case.overwrite_case_ordinal_for_test(inventory.case_count());
        assert!(matches!(
            authority.authenticated_same_group_target_case_view(
                &family,
                &context,
            ...
            Err(GeneratedAffineResidualCaseAuthorityError::WrongTargetCase)
        ));
        let mut wrong_group = handle;
        wrong_group.overwrite_group_ordinal_for_test(inventory.group_count());
        assert!(matches!(
            authority.authenticated_same_group_target_case_view(
                &family,
                &context,
            ...
            Err(GeneratedAffineResidualCaseAuthorityError::WrongTargetGroup)
        ));
        let mut wrong_ordinal = handle;
        wrong_ordinal.overwrite_ordinal_within_group_for_test(baseline.len());
        assert!(matches!(
            authority.authenticated_same_group_target_case_view(
                &family,
                &context,
            ...
            Err(GeneratedAffineResidualCaseAuthorityError::WrongTargetOrdinal)
        ));
        let source_position = baseline
            .group
            .case_ordinals()
            .iter()
            .position(|&ordinal| ordinal == source_case_ordinal)
            .unwrap();
        let source_handle = baseline
            .target(
                source_position,
                GeneratedAffineResidualSameGroupTargetHandleLimits::default(),
            )
            .unwrap();
        let mut wrong_geometry = handle;
        wrong_geometry.overwrite_constants_for_test(source_handle.constants());
        assert!(matches!(
            authority.authenticated_same_group_target_case_view(
                &family,
                &context,
            ...
            Err(GeneratedAffineResidualCaseAuthorityError::TargetGeometryMismatch)
        ));

        let wrong_family = equal_mass_two_loop_family(
            "v2-affine-inventory-same-group-target-wrong-family-private",
        );
        assert!(matches!(
            authority.same_group_target_cases(
                &wrong_family,
                &context,
                GeneratedAffineResidualSameGroupTargetCasesLimits::default(),
            ),
        ...
        let wrong_context = ParametricCoefficientContext::try_new(
            family.coefficient_context(),
            "v2-affine-inventory-same-group-target-wrong-context-private",
            context.index_count(),
        )
        .unwrap();
        assert!(matches!(
        ...
        let debug = format!("{baseline:?} {handle:?} {resolved:?}");
        assert!(debug.contains("<redacted>"));
        assert!(!debug.contains("m2"));
        drop(independent);
        drop(boolean);
        drop(inventory);
        drop(exact_collection);
        drop(baseline);
        authority.replay(&family, &context).unwrap();
    }

    #[test]
    fn same_group_target_and_source_row_borrows_are_concurrent_and_replayable() {
        let (family, context, boolean, inventory) =
            two_loop_fixture("v2-affine-inventory-same-group-concurrent-private");
        let inventory = Arc::new(inventory);
        let source_case_ordinal = inventory
            .groups
            .iter()
            .max_by_key(|group| group.case_ordinals.len())
            .unwrap()
        ...
        thread::scope(|scope| {
            ...
            for worker in 0..4usize {
                ...
                joins.push(scope.spawn(move || {
                    let source_row_ordinal = worker % authority.source_row_count();
                    let row = authority
                        .authenticated_source_row_view(
                            family,
                            context,
                    ...
                    let targets = authority
                        ...
                        .unwrap();
                    let target_position = worker % targets.len();
                    let handle = targets
                        .target(
                            target_position,
                            GeneratedAffineResidualSameGroupTargetHandleLimits::default(),
                    ...
                    let resolved = authority
                        ...
                        .unwrap();
                    assert_eq!(resolved.target().ordinal(), handle.case_ordinal());
                    assert_eq!(resolved.target().group_ordinal(), targets.group_ordinal());
                    assert!(resolved.stats().authentication().case_authentications() > 0);
                }));
            }
            for join in joins {
                join.join().unwrap();
    ...
    #[test]
    fn case_authority_replay_borrowed_scope_preflight_and_panic_boundary_are_fail_closed() {
        let (family, context, _, inventory) =
            one_loop_fixture("v2-affine-inventory-authority-replay-boundary-private");
        let inventory = Arc::new(inventory);
        let authority = GeneratedAffineResidualCaseAuthority::try_new(
            &family,
            &context,
        ...
        // Scope admission reads the family's cached fingerprint by borrow and
        // must reject before the replay compiler performs any allocation.
        let exact_scope = authority.stats().scope_comparison_bytes();
        assert!(exact_scope > 0);
        let mut scope_limited = authority.clone();
        scope_limited.limits.max_scope_comparison_bytes = exact_scope - 1;
        reset_inventory_compilations_for_test();
        assert!(matches!(
            scope_limited.replay(&family, &context),
            Err(GeneratedAffineResidualCaseAuthorityError::ResourceLimit {
                resource: "scope comparison bytes",
                requested,
                limit,
            }) if requested == exact_scope && limit == exact_scope - 1
        ));
        assert_eq!(inventory_compilations_for_test(), 0);

        // Exercise the authority's own unwind boundary before the nested
        // inventory replay boundary.  The injected panic is thread-local and
        // consumed before unwinding, so parallel tests cannot observe it.
        CASE_AUTHORITY_REPLAY_PANIC_FOR_TEST.with(|panic_next| panic_next.set(true));
        reset_inventory_compilations_for_test();
        let panic_error = authority.replay(&family, &context).unwrap_err();
        assert!(matches!(
            panic_error,
    ...
    #[test]
    fn natural_s2_exposes_only_boolean_empty_and_initial_actionable_sources() {
        let (family, context, boolean, inventory) =
            two_loop_fixture("v2-affine-inventory-two-loop-private");
        assert_eq!(
            natural_terminal_census(&context, &inventory),
            NaturalTerminalCensus {
                boolean_proved_empty: 2,
                initial_affine_actionable: 5,
                initial_equal_zero_atoms: 4,
                initial_nonzero_atoms: 4,
                guard_discharged_nonzero_integer_constants: 2,
                guard_free_index_dependent: 5,
                initial_cases: 5,
                ..NaturalTerminalCensus::default()
            }
        );
        assert_eq!(inventory.terminal_count(), 7);
        assert_eq!(inventory.case_count(), 5);
        assert_eq!(inventory.group_count(), 4);
        for ordinal in 0..inventory.terminal_count() {
            let terminal = inventory
                .authenticated_terminal_view(&context, ordinal)
                .unwrap();
            let direct = boolean.authenticated_terminal_view(ordinal).unwrap();
            assert_eq!(terminal.locator().boolean_record_ordinal(), ordinal);
            assert_eq!(
                terminal.locator().source_work_item_ordinal(),
                direct.locator().source_work_item_ordinal()
            );
            assert_eq!(
                terminal.locator().local_terminal_ordinal(),
                direct.locator().terminal_ordinal()
            );
            match terminal.source() {
                GeneratedAffineResidualInventoryTerminalSourceView::BooleanProvedEmpty(source) => {
                    ...
                    assert_initial_boolean_atoms_are_positional(source);
                    match direct.source() {
                        GeneratedAffineResidualBooleanTerminalSourceView::InitialBoolean(
                            direct_source,
                        ) => assert_initial_boolean_sources_match(source, direct_source),
                        _ => panic!("Boolean-empty S2 record changed its exact source variant"),
                    }
                }
                GeneratedAffineResidualInventoryTerminalSourceView::InitialAffineActionable(
                    source,
                    ...
                    let case = case_for_terminal(&context, &inventory, ordinal);
                    assert_eq!(case.locator(), terminal.locator());
                    assert_eq!(case.constants().len(), source.affine_map().ambient_arity());
                    for (position, constant) in case.constants().iter().enumerate() {
                        assert_eq!(Some(constant), source.affine_map().constant(position));
                    }
                    match case.source() {
                        GeneratedAffineResidualInventoryCaseSourceView::Initial(case_source) => {
                            assert!(std::ptr::eq(source.affine_map(), case_source.affine_map()));
                            assert_initial_guard_sequences_match(
                                source.guards(),
                                case_source.guards(),
                            );
                        }
                        _ => panic!("initial actionable S2 case changed source authority"),
                    }
                    match direct.source() {
                        GeneratedAffineResidualBooleanTerminalSourceView::InitialBoolean(
                            direct_source,
                            ...
                            assert_eq!(
                                direct_source.outcome(),
                                GeneratedAffineInitialGlobalBooleanTerminalOutcome::ReadyForAffineRecognition
                            );
                            assert_initial_boolean_atoms_are_positional(direct_source);
                        }
                        _ => panic!("actionable S2 record changed its exact source variant"),
                    }
                }
                _ => panic!("natural S2 produced an unmeasured terminal source variant"),
            }
        }
        for ordinal in 0..inventory.group_count() {
            inventory
    ...
    #[test]
    fn natural_shifted_u2_routes_initial_and_prior_unsupported_without_cases() {
        let (family, context, boolean, inventory) =
            shifted_unsupported_fixture("v2-affine-inventory-shifted-u2-private");
        assert_eq!(
            natural_terminal_census(&context, &inventory),
            NaturalTerminalCensus {
                initial_affine_unsupported: 1,
                initial_unsupported_reasons: 1,
                ..NaturalTerminalCensus::default()
            }
        );
        assert_eq!(inventory.terminal_count(), 1);
        assert_eq!(inventory.case_count(), 0);
        assert_eq!(inventory.group_count(), 0);
        let terminal = inventory.authenticated_terminal_view(&context, 0).unwrap();
        assert_eq!(
            terminal.outcome(),
            GeneratedAffineResidualInventoryTerminalOutcome::AffineUnsupported
        );
        match terminal.source() {
            GeneratedAffineResidualInventoryTerminalSourceView::InitialAffineUnsupported(
                ...
                assert_eq!(source.reason_count(), 1);
                assert!(source.reason(0).is_some());
                assert!(source.reason(1).is_none());
            }
            _ => panic!("natural shifted U2 changed its initial unsupported source"),
        }
        let direct = boolean.authenticated_terminal_view(0).unwrap();
        match direct.source() {
            GeneratedAffineResidualBooleanTerminalSourceView::InitialBoolean(source) => {
                assert_eq!(
                    source.outcome(),
                ...
                assert_eq!(source.equal_zero_atom_count(), 2);
                assert_eq!(source.nonzero_atom_count(), 0);
                assert_initial_boolean_atoms_are_positional(source);
            }
            _ => panic!("natural shifted U2 changed its Boolean source"),
        }
        inventory.replay(&family, &context).unwrap();
        exercise_positive_outer_limit_table(
            &family,
            &context,
            &boolean,
            &inventory,
        ...
        let (prior_family, prior_context, prior_boolean, prior_inventory) =
            prior_shifted_unsupported_fixture("v2-affine-inventory-prior-shifted-u2-private");
        assert_eq!(
            natural_terminal_census(&prior_context, &prior_inventory),
            NaturalTerminalCensus {
                prior_unsupported: 1,
                prior_equal_zero_atoms: 2,
                prior_unsupported_reasons: 1,
                ..NaturalTerminalCensus::default()
            }
        );
        assert_eq!(prior_inventory.terminal_count(), 1);
        assert_eq!(prior_inventory.case_count(), 0);
        assert_eq!(prior_inventory.group_count(), 0);
        let prior_terminal = prior_inventory
            .authenticated_terminal_view(&prior_context, 0)
            .unwrap();
        assert_eq!(
            prior_terminal.outcome(),
            GeneratedAffineResidualInventoryTerminalOutcome::AffineUnsupported
        );
        let prior_direct = prior_boolean.authenticated_terminal_view(0).unwrap();
        match (prior_terminal.source(), prior_direct.source()) {
            (
                GeneratedAffineResidualInventoryTerminalSourceView::PriorUnsupported(source),
                GeneratedAffineResidualBooleanTerminalSourceView::PriorUnsupported(direct_source),
            ) => {
                assert_eq!(
                    source.atom_count(GeneratedAffineResidualBooleanAtomPolarity::EqualZero),
                    2
                ...
                assert_eq!(source.unsupported_reason_count(), 1);
                let symbolic_nu_atom = source
                    .atom(GeneratedAffineResidualBooleanAtomPolarity::EqualZero, 0)
                    .unwrap();
                assert_symbolic_nu_locus_has_no_concrete_i64_witness(
                    &prior_context,
                    symbolic_nu_atom.polynomial(),
                );
                let integer_compatible_atom = source
                    .atom(GeneratedAffineResidualBooleanAtomPolarity::EqualZero, 1)
                    .unwrap();
                assert!(
                    prior_context
                        .specialize_polynomial(
                            integer_compatible_atom.polynomial(),
                            &[1, 1, 0],
                            ParametricArithmeticLimits::default(),
                        )
                        .unwrap()
                        .is_zero(),
                    "the ordinary n1=1 atom is concrete; only the symbolic nu atom blocks the conjunction"
                );
                assert_prior_unsupported_sources_match(source, direct_source);
            }
            _ => panic!("natural shifted U2 changed its prior unsupported source"),
        }
        // The structural monomial proof above covers every possible i64
        // assignment, while this concrete sector point confirms the other
        // atom alone can vanish. The symbolic terminal remains meaningful and
        // is therefore authenticated positionally, but it cannot be reached
        // through the concrete point-classification API.
        assert_eq!(
            prior_inventory
                .classification_for_indices(
                    &prior_family,
        ...
        prior_inventory
            .replay(&prior_family, &prior_context)
            .unwrap();
        exercise_positive_outer_limit_table(
            &prior_family,
            &prior_context,
    ...
    #[test]
    fn natural_prior_001_exposes_only_observed_domain_and_leak_exceptions() {
        let (family, context, boolean, inventory) =
            prior_two_loop_fixture("v2-affine-inventory-prior-effective-private", "001");
        let inventory = Arc::new(inventory);
        assert_eq!(
            natural_terminal_census(&context, &inventory),
            NaturalTerminalCensus {
                prior_exceptional_domain: 1,
                prior_exceptional_leak: 2,
                exceptional_predicates: 6,
                prior_exceptional_domain_cases: 1,
                prior_exceptional_leak_cases: 2,
                ..NaturalTerminalCensus::default()
            }
        );
        assert_eq!(inventory.terminal_count(), 3);
        assert_eq!(inventory.case_count(), 3);
        assert_eq!(inventory.group_count(), 1);
        let mut domain_point_witness = None;
        let mut leak_point_witness = None;
        for ordinal in 0..inventory.terminal_count() {
            let terminal = inventory
                .authenticated_terminal_view(&context, ordinal)
                .unwrap();
            ...
            let direct = boolean.authenticated_terminal_view(ordinal).unwrap();
            let case = case_for_terminal(&context, &inventory, ordinal);
            match (terminal.source(), direct.source(), case.source()) {
                (
                    GeneratedAffineResidualInventoryTerminalSourceView::PriorExceptionalDomain(
                        terminal_source,
                    ),
                    GeneratedAffineResidualBooleanTerminalSourceView::PriorExceptionalDomain(
                        direct_source,
                    ),
                    GeneratedAffineResidualInventoryCaseSourceView::PriorExceptionalDomain(
                        case_source,
                    ),
                ) => {
                    if domain_point_witness.is_none() {
                        domain_point_witness =
                            case_full_point_chain_witness(&family, &context, &inventory, case);
                    }
                    assert_prior_exceptional_terminal_and_case_match(
                        terminal_source,
                        direct_source,
                    );
                    assert_prior_exceptional_terminal_and_case_match(terminal_source, case_source);
                }
                (
                    GeneratedAffineResidualInventoryTerminalSourceView::PriorExceptionalLeak(
                        terminal_source,
                    ),
                    GeneratedAffineResidualBooleanTerminalSourceView::PriorExceptionalLeak(
                        direct_source,
                    ),
                    GeneratedAffineResidualInventoryCaseSourceView::PriorExceptionalLeak(
                        case_source,
                    ),
                ) => {
                    if leak_point_witness.is_none() {
                        leak_point_witness =
                            case_full_point_chain_witness(&family, &context, &inventory, case);
                    }
                    assert_prior_exceptional_terminal_and_case_match(
                ...
                _ => panic!("natural prior 001 changed its measured source partition"),
            }
        }
        let domain_point_witness =
            domain_point_witness.expect("natural prior domain case has one concrete i64 witness");
        assert!(leak_point_witness.is_some());
        let domain_classification = inventory
            .classification_for_indices(
                &family,
                &context,
                &domain_point_witness,
            ...
            .unwrap();
        let GeneratedAffineResidualInventoryPointDisposition::Actionable {
            case_ordinal: domain_case_ordinal,
            ..
        } = domain_classification.disposition()
        else {
            panic!("natural prior domain witness must retain its actionable case");
        };
        assert_inventory_point_exact_and_every_positive_one_below(
            &family,
            &context,
    ...
    #[test]
    fn natural_prior_011_exposes_actionable_and_domain_positional_views() {
        let (family, context, boolean, inventory) =
            prior_two_loop_fixture("v2-affine-inventory-prior-011-private", "011");
        assert_eq!(
            natural_terminal_census(&context, &inventory),
            NaturalTerminalCensus {
                prior_actionable: 2,
                prior_exceptional_domain: 2,
                guard_discharged_nonzero_integer_constants: 2,
                guard_free_index_dependent: 4,
                exceptional_predicates: 2,
                prior_actionable_cases: 2,
                prior_exceptional_domain_cases: 2,
                ..NaturalTerminalCensus::default()
            }
        );
        assert_eq!(inventory.terminal_count(), 4);
        assert_eq!(inventory.case_count(), 4);
        assert_eq!(inventory.group_count(), 3);
        let mut actionable_point_witness = false;
        let mut domain_point_witness = false;
        for ordinal in 0..inventory.terminal_count() {
            let terminal = inventory
                .authenticated_terminal_view(&context, ordinal)
                .unwrap();
            ...
            match (terminal.source(), direct.source(), case.source()) {
                (
                    GeneratedAffineResidualInventoryTerminalSourceView::PriorActionable(
                        terminal_source,
                    ),
                    GeneratedAffineResidualBooleanTerminalSourceView::PriorActionable(
                        direct_source,
                    ),
                    GeneratedAffineResidualInventoryCaseSourceView::PriorActionable(case_source),
                ) => {
                    if !actionable_point_witness {
                        actionable_point_witness =
                            case_full_point_chain_witness(&family, &context, &inventory, case)
                                .is_some();
                    }
                    assert_prior_actionable_terminal_and_case_match(terminal_source, direct_source);
                    assert_prior_actionable_terminal_and_case_match(terminal_source, case_source);
                }
                (
                    GeneratedAffineResidualInventoryTerminalSourceView::PriorExceptionalDomain(
                        terminal_source,
                ...
                _ => panic!("natural prior 011 changed its measured source partition"),
            }
        }
        assert!(actionable_point_witness);
        assert!(domain_point_witness);
        inventory.replay(&family, &context).unwrap();
        exercise_positive_outer_limit_table(
            &family,
            &context,
    ...
    #[test]
    fn bounded_tables_name_every_outer_axis_without_claiming_unobserved_variants() {
        let covered_tables = [
            INITIAL_COUNT_LIMIT_AXES,
            SOURCE_BINDING_LIMIT_AXES,
            GEOMETRY_LIMIT_AXES,
            MEMORY_LIMIT_AXES,
            PAYLOAD_LIMIT_AXES,
            NATURAL_VARIANT_LIMIT_AXES,
        ];
        for &axis in ALL_OUTER_LIMIT_AXES {
            let occurrences = covered_tables
                .iter()
            ...
            if matches!(
                ...
                assert_eq!(
                    occurrences, 0,
                    "currently unobserved axis {axis:?} must not be presented as natural coverage"
                );
            } else {
                assert_eq!(
                    occurrences, 1,
                    "natural positive outer axis {axis:?} must have one bounded table"
                );
            }
        }
    }
    ...
    #[test]
    fn natural_s2_count_limits_accept_exact_and_reject_one_below() {
        let (family, context, source, baseline) =
            two_loop_fixture("v2-affine-inventory-count-limits-s2-private");
        exercise_positive_outer_limit_table(
            &family,
            &context,
            &source,
    ...
    #[test]
    fn natural_s2_source_binding_limits_accept_exact_and_reject_one_below() {
        let (family, context, source, baseline) =
            two_loop_fixture("v2-affine-inventory-source-limits-s2-private");
        exercise_positive_outer_limit_table(
            &family,
            &context,
            &source,
    ...
    #[test]
    fn natural_s2_geometry_limits_accept_exact_and_reject_one_below() {
        let (family, context, source, baseline) =
            two_loop_fixture("v2-affine-inventory-geometry-limits-s2-private");
        exercise_positive_outer_limit_table(
            &family,
            &context,
            &source,
    ...
    #[test]
    fn natural_s2_memory_limits_accept_exact_and_reject_one_below() {
        let (family, context, source, baseline) =
            two_loop_fixture("v2-affine-inventory-memory-limits-s2-private");
        exercise_positive_outer_limit_table(
            &family,
            &context,
            &source,
    ...
    #[test]
    fn natural_s2_payload_limits_accept_exact_and_reject_one_below() {
        let (family, context, source, baseline) =
            two_loop_fixture("v2-affine-inventory-payload-limits-s2-private");
        exercise_positive_outer_limit_table(
            &family,
            &context,
            &source,
    ...
    #[test]
    fn exact_source_arc_identity_precedes_deep_payload_comparison() {
        let fixture_name = "v2-affine-inventory-source-identity-equal-private";
        let (family, context, source, inventory) = one_loop_fixture(fixture_name);
        let (independent_family, independent_context, independent_source, independent) =
            one_loop_fixture(fixture_name);
        assert_eq!(
            family.fingerprint_ref(),
            independent_family.fingerprint_ref()
        );
        assert_eq!(context.fingerprint(), independent_context.fingerprint());
        assert!(!Arc::ptr_eq(&source, &independent_source));
        let inventory = Arc::new(inventory);
        let independent = Arc::new(independent);
        assert!(
            !inventory
                .payload_eq_checked(independent.as_ref(), &independent_context)
                .unwrap()
        );
        let authority = GeneratedAffineResidualCaseAuthority::try_new(
            &family,
        ...
        .unwrap();
        assert!(authority.same_inventory_allocation(&inventory));
        assert!(!authority.same_inventory_allocation(&independent));
        inventory.replay(&family, &context).unwrap();
    }

    #[test]
    fn replay_retains_the_exact_source_after_external_handles_are_dropped() {
        let (family, context, source, inventory) =
            one_loop_fixture("v2-affine-inventory-source-drop-private");
        assert!(Arc::strong_count(&source) >= 2);
        drop(source);
        inventory.replay(&family, &context).unwrap();
    }

    #[test]
    fn same_source_coherent_comparison_tamper_is_rejected_by_raw_census() {
        let (family, context, source, mut left) =
            one_loop_fixture("v2-affine-inventory-pair-raw-census-private");
        let mut right = GeneratedAffineResidualCaseInventoryCompiler::compile(
            &family,
            &context,
            source,
            left.limits(),
        ...
        .unwrap();
        assert!(left.payload_eq_checked(&right, &context).unwrap());
        assert!(left.stats.recursive_child_comparison_units > 0);
        left.stats.recursive_child_comparison_units -= 1;
        left.stats.payload_comparison_units -= 1;
        right.stats.recursive_child_comparison_units -= 1;
        right.stats.payload_comparison_units -= 1;
        assert!(left.payload_eq_checked(&right, &context).is_err());
    }

    #[test]
    fn recursive_child_comparison_admission_boundaries_precede_outer_allocation() {
        let (family, context, source, baseline) =
            one_loop_fixture("v2-affine-inventory-recursive-admission-private");
        let stats = baseline.stats();
        let unit_demand = stats.recursive_child_comparison_units_admission_demand();
        let byte_demand = stats.recursive_child_comparison_bytes_admission_demand();
        let bit_demand = stats.recursive_child_comparison_integer_bits_admission_demand();
        assert!(unit_demand > 0 && byte_demand > 0 && bit_demand > 0);
        assert!(stats.recursive_child_comparison_units() <= unit_demand);
        assert!(stats.recursive_child_comparison_bytes() <= byte_demand);
        assert!(stats.recursive_child_comparison_integer_bits() <= bit_demand);

        let mut exact_limits = baseline.limits();
        exact_limits.max_recursive_child_comparison_units = unit_demand;
        exact_limits.max_recursive_child_comparison_bytes = byte_demand;
        exact_limits.max_recursive_child_comparison_integer_bits = bit_demand;
        let exact = GeneratedAffineResidualCaseInventoryCompiler::compile(
            &family,
            &context,
            Arc::clone(&source),
        ...
        assert!(exact.stats().recursive_child_comparison_units() <= unit_demand);
        assert!(exact.stats().recursive_child_comparison_bytes() <= byte_demand);
        assert!(exact.stats().recursive_child_comparison_integer_bits() <= bit_demand);
        exact.replay(&family, &context).unwrap();

        macro_rules! rejects_one_below_before_outer_reserve {
            ($field:ident, $demand:expr, $resource:literal) => {{
                let mut limits = baseline.limits();
                ...
                assert_eq!(inventory_compilations_for_test(), 1);
                assert_eq!(inventory_outer_reserves_for_test(), 0);
                assert_eq!(inventory_ready_consumptions_for_test(), 0);
            }};
        }
        rejects_one_below_before_outer_reserve!(
            max_recursive_child_comparison_units,
            unit_demand,
            "recursive child comparison units"
        );
        rejects_one_below_before_outer_reserve!(
            max_recursive_child_comparison_bytes,
            byte_demand,
            "recursive child comparison bytes"
        );
        rejects_one_below_before_outer_reserve!(
            max_recursive_child_comparison_integer_bits,
            bit_demand,
            "recursive child comparison integer bits"
        );
    }
    ...
    #[test]
    fn multiple_ready_children_replay_at_the_exact_admission_aware_peak() {
        let (family, context, source, baseline) =
            two_loop_fixture("v2-affine-inventory-multiple-ready-exact-replay-private");
        assert!(baseline.stats.initial_affine_children >= 5);
        let mut limits = baseline.limits();
        // Increase only a conservative retained branch-envelope allowance;
        // the concrete source and all actual child work stay unchanged.  The
        // aggregate N-child admission must then dominate the one-child-at-a-
        // time sequential compilation envelope.
        limits.branch.max_retained_atom_context_fingerprint_bytes += 100_000_000_000_000_000;
        let mut inventory = GeneratedAffineResidualCaseInventoryCompiler::compile(
            &family, &context, source, limits,
        )
        .unwrap();
        assert!(
            inventory
                .stats
                .compilation_owned_logical_peak_admission_demand
                > inventory.stats.compilation_owned_logical_peak,
            "fixture must make admission {} dominate observed {}",
            inventory
                .stats
                .compilation_owned_logical_peak_admission_demand,
            inventory.stats.compilation_owned_logical_peak,
        );
        inventory.limits.max_replay_owned_logical_peak = inventory.stats.replay_owned_logical_peak;
        inventory.replay(&family, &context).unwrap();
    }

    #[test]
    fn terminal_and_geometry_tampering_fail_exact_replay() {
        let (family, context, _, mut terminal_tamper) =
            two_loop_fixture("v2-affine-inventory-terminal-tamper-private");
        terminal_tamper.terminals[0].ordinal = 1;
        assert!(terminal_tamper.replay(&family, &context).is_err());
        assert!(matches!(
            GeneratedAffineResidualCaseAuthority::try_new(
                &family,
                &context,
                Arc::new(terminal_tamper),
                0,
                GeneratedAffineResidualCaseAuthorityLimits::default(),
            ),
            Err(GeneratedAffineResidualCaseAuthorityError::Inventory(_))
        ));

        let (family, context, _, mut geometry_tamper) =
            two_loop_fixture("v2-affine-inventory-geometry-tamper-private");
        let offset = geometry_tamper
            .groups
            .iter_mut()
            .find_map(|group| group.anchor_offsets.first_mut())
            .and_then(|offset| offset.first_mut())
            .expect("two-loop fixture retains an affine offset");
        *offset = offset.clone() + Integer::from(1);
        assert!(geometry_tamper.replay(&family, &context).is_err());
        assert!(matches!(
            GeneratedAffineResidualCaseAuthority::try_new(
                &family,
                &context,
                Arc::new(geometry_tamper),
                0,
                GeneratedAffineResidualCaseAuthorityLimits::default(),
            ),
            Err(GeneratedAffineResidualCaseAuthorityError::Inventory(_))
    ...
    #[test]
    fn duplicated_ready_child_binding_cannot_orphan_a_retained_child() {
        let (family, context, _, mut inventory) =
            two_loop_fixture("v2-affine-inventory-ready-bijection-tamper-private");
        let mut ready_count = 0usize;
        for record in &mut inventory.terminals {
            if let GeneratedAffineResidualInventoryTerminalBinding::InitialAffineTerminal {
                child_ordinal,
                ..
            } = &mut record.binding
        ...
        assert!(
            ready_count >= 2,
            "fixture must exercise a nontrivial child order"
        );
        assert!(inventory.replay(&family, &context).is_err());
    }

    #[test]
    fn coherent_downward_memory_stats_tamper_fails_before_rebuild() {
        let (family, context, _, mut inventory) =
            one_loop_fixture("v2-affine-inventory-downward-stats-tamper-private");
        assert!(inventory.stats.child_retained_owned_logical_bytes > 0);
        assert!(inventory.stats.retained_owned_logical_bytes > 0);
        assert!(inventory.stats.replay_owned_logical_peak > 0);
        inventory.stats.child_retained_owned_logical_bytes -= 1;
        inventory.stats.retained_owned_logical_bytes -= 1;
        inventory.stats.replay_owned_logical_peak -= 1;
        reset_inventory_ready_consumptions_for_test();
        reset_inventory_compilations_for_test();
        reset_inventory_outer_reserves_for_test();
        assert!(inventory.replay(&family, &context).is_err());
    ...
    #[test]
    fn exact_replay_peak_passes_and_coherent_headroom_boundaries_precede_allocation() {
        let (family, context, boolean, baseline) =
            one_loop_fixture("v2-affine-inventory-replay-headroom-private");
        let mut exact_limits = baseline.limits();
        exact_limits.max_replay_owned_logical_peak = baseline.stats().replay_owned_logical_peak();
        let mut exact = GeneratedAffineResidualCaseInventoryCompiler::compile(
            &family,
            &context,
            boolean,
            exact_limits,
        ...
        reset_inventory_outer_reserves_for_test();
        exact.replay(&family, &context).unwrap();
        assert!(inventory_ready_consumptions_for_test() > 0);
        assert_eq!(inventory_compilations_for_test(), 1);
        assert_eq!(inventory_outer_reserves_for_test(), 1);
        ...
        let prospective_fixed_retained = [
            ...
            exact
                .terminals
                .len()
                .checked_mul(size_of::<GeneratedAffineResidualInventoryTerminalRecord>())
                .unwrap(),
            exact
                .stats
                .child_retained_owned_logical_bytes_admission_demand,
        ...
        .sum::<usize>();
        assert!(
            exact.stats.compilation_owned_logical_peak > prospective_fixed_retained,
            "fixture must have a Ready-derived live peak above the fixed envelope"
        );
        exact.stats.compilation_owned_logical_peak -= 1;
        exact.stats.compilation_owned_logical_peak_admission_demand =
            prospective_fixed_retained.max(exact.stats.compilation_owned_logical_peak);
        exact.stats.replay_owned_logical_peak = exact.stats.retained_owned_logical_bytes
            + exact.stats.compilation_owned_logical_peak_admission_demand;
        exact.limits.max_replay_owned_logical_peak = exact.stats.replay_owned_logical_peak;
        reset_inventory_ready_consumptions_for_test();
        reset_inventory_compilations_for_test();
        reset_inventory_outer_reserves_for_test();
        assert!(matches!(
            exact.replay(&family, &context),
            Err(GeneratedAffineResidualCaseInventoryError::ResourceLimit {
                resource: "compilation owned logical peak",
                ..
            })
        ));
        assert!(
            inventory_ready_consumptions_for_test() < exact.stats.initial_affine_children,
            "the peak-causing Ready child must not be consumed"
        );
        assert_eq!(inventory_compilations_for_test(), 1);
        assert_eq!(inventory_outer_reserves_for_test(), 1);

        let mut below_fixed_limits = exact.limits;
        below_fixed_limits.max_compilation_owned_logical_peak = prospective_fixed_retained - 1;
        reset_inventory_ready_consumptions_for_test();
        reset_inventory_compilations_for_test();
        reset_inventory_outer_reserves_for_test();
        assert!(matches!(
            GeneratedAffineResidualCaseInventoryCompiler::compile(
                &family,
                &context,
                Arc::clone(&exact.source_boolean_cover),
                below_fixed_limits,
            ),
            Err(GeneratedAffineResidualCaseInventoryError::ResourceLimit {
                resource: "compilation owned logical peak",
    ...
    #[test]
    fn shared_inventory_replays_concurrently_on_four_threads() {
        let (family, context, _, inventory) =
            one_loop_fixture("v2-affine-inventory-concurrent-replay-private");
        ...
        let handles: Vec<_> = (0..4)
            .map(|_| {
                ...
                thread::spawn(move || inventory.replay(&family, &context))
            })
            .collect();
    ...
    #[test]
    fn gmp_and_geometry_peak_boundary_formulas_are_pinned() {
        assert_eq!(gmp_logical_bytes(0, 0).unwrap(), 0);
        assert_eq!(gmp_logical_bytes(1, 0).unwrap(), size_of::<usize>());
        assert_eq!(gmp_logical_bytes(2, 0).unwrap(), 2 * size_of::<usize>() + 1);
        assert_eq!(gmp_logical_bytes(2, 129).unwrap(), 34);
        assert!(gmp_logical_bytes(usize::MAX, usize::MAX).is_err());

        // The already-audited pre-offset Geometry envelope plus its case-ref
        // slot dominates the post-commit Case wrapper even at zero GMP bits.
        assert!(
            size_of::<Geometry>() + size_of::<usize>()
                >= size_of::<GeneratedAffineResidualInventoryCase>()
        );
        assert!(
            size_of::<Vec<Integer>>() + size_of::<usize>()
                >= size_of::<usize>() + size_of::<Vec<Integer>>()
        );
        assert!(
            size_of::<GroupBuilder>() >= size_of::<GeneratedAffineResidualContiguousCaseGroup>()
        );
        for total in 0..8usize {
            for finished in 0..=total {
                let sequential_finish_shape = finished
                    * size_of::<GeneratedAffineResidualContiguousCaseGroup>()
                    + (total - finished) * size_of::<GroupBuilder>();
                assert!(sequential_finish_shape <= total * size_of::<GroupBuilder>());
            }
        }
    }
}
