...
pub(super) fn parse(arguments: impl Iterator<Item = OsString>) -> Result<Command, ArgError> {
    ...
    while let Some(option) = arguments.next() {
        ...
        match option.as_str() {
            ...
            "--sectors" => {
                let value = next_utf8_value(&mut arguments, "--sectors")?;
                set_once(&mut sectors, "--sectors", parse_sectors(value)?)?;
            }
            "--n-cores" => {
    ...
    Ok(Command::FamilySolve(FamilySolveArgs {
...
fn parse_sectors(value: String) -> Result<Vec<Vec<bool>>, ArgError> {
    if value.is_empty() {
        return Err(ArgError::InvalidValue {
            option: "--sectors",
            value,
            expected: "nonempty 0/1 sector strings separated by commas",
        });
    }
    value
        .split(',')
        .map(|token| {
            if token.is_empty() || !token.bytes().all(|byte| byte == b'0' || byte == b'1') {
                return Err(ArgError::InvalidValue {
                    option: "--sectors",
                    value: value.clone(),
                    expected: "nonempty 0/1 sector strings separated by commas",
            ...
            Ok(token.bytes().map(|byte| byte == b'1').collect())
        })
        .collect()
}
