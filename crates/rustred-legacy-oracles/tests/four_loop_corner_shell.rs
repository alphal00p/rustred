...
use rustred::{CoefficientContext, MasterProduct};
use rustred_legacy_oracles::{
    ...
    FOUR_LOOP_CORNER_SHELL_GLOBAL_COLUMN_BOUND, FOUR_LOOP_CORNER_SHELL_RAW_ROWS,
    FourLoopBoundaryHaloCensusKey, FourLoopCornerColumnId, FourLoopCornerRawRowId,
    FourLoopCornerShellCertificate, FourLoopCornerShellConfig, FourLoopCornerShellError,
    FourLoopCornerShellStatus, FourLoopGenuineCornerType, FourLoopReferenceTopology,
    MassiveVacuumMaster,
};
...
fn native_160_row_corner_shell_is_a_replayable_complete_certificate() {
    ...
    assert!(matches!(
        ...
        Err(FourLoopCornerShellError::BoundaryHalo(
            rustred_legacy_oracles::FourLoopBoundaryHaloError::ResourceLimit {
    ...
    assert_eq!(
        certificate
            .raw_row_ids()
            .iter()
            .filter(|id| id.corner_type().reference_topology()
