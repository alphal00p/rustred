...
use super::model::{ProductMomentGuard, SingletonProductBlock};
...
impl PartialMomentKey {
    pub(super) fn radial_powers(&self, loop_count: usize) -> &[u64] {
        &self.powers[..loop_count]
    }

    pub(super) fn cross_powers(&self, loop_count: usize) -> &[u64] {
        &self.powers[loop_count..]
    }
}
...
impl<'chart> PartialAngularEvaluator<'chart> {
        ...
        while let Some(((measure, key), coefficient)) = pending.pop_last() {
            ...
            for edge in 0..self.edges.len() {
                ...
                let multiplicity = i64::try_from(multiplicity).map_err(|_| {
                    FactorizedProductMomentError::RankCoefficientOverflow { rank: multiplicity }
                })?;
    ...
    fn select_singleton(
        &self,
        cross: &[u64],
    ) -> Result<Option<(usize, u64)>, FactorizedProductMomentError> {
    ...
    fn measure(&self, cross: &[u64]) -> Result<u64, FactorizedProductMomentError> {
...
fn accumulate_pending(
    ...
    map: &mut BTreeMap<(u64, PartialMomentKey), Coefficient>,
    measure: u64,
...
fn accumulate_state(
    ...
    accumulate_generic(context, map, key, coefficient, budget, limits, additions)
}
...
fn decrement(value: &mut u64, resource: &'static str) -> Result<(), FactorizedProductMomentError> {
    *value = value
        .checked_sub(1)
        .ok_or(FactorizedProductMomentError::ResourceCountOverflow { resource })?;
    Ok(())
