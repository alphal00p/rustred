//! Exact two-dot numerator endpoint on the factorized four-line face.
//!
//! This module owns one `S4` orbit of `J(0,-1,2,2,1,1)`.  A complete
//! depth-one search selects nine of the 63 translated ordinary K6 rows.
//! Production reprojects only those selected rows, which removes a spurious
//! dimension guard retained by the complete elimination path.  Every compact
//! rule child is an authenticated factorization terminal; no deeper numerator
//! or inequivalent dot placement is included in this singleton cell.
...
pub(super) const TWO_DOT_NUMERATOR_PIVOT: [i64; 6] = [0, -1, 1, 1, 0, 0];
const SEARCH_DEPTH: usize = 1;

pub(super) struct FactorizedTwoDotSelectionWitness {
    pub(super) complete_sources: SourceViewBatch,
    pub(super) complete_rule: ParametricRule,
}

pub(super) struct FactorizedTwoDotEndpointBuild {
    ...
    pub(super) selection_witness: Option<FactorizedTwoDotSelectionWitness>,
}

pub(in super::super) fn derive_factorized_two_dot_numerator_endpoint()
-> Result<(IndexedCoefficientContext, RuleCell), ArtifactError> {
    let build = derive_factorized_two_dot_endpoint_build(false)?;
    Ok((build.context, build.endpoint))
}

pub(super) fn derive_factorized_two_dot_endpoint_build(
    retain_selection_witness: bool,
) -> Result<FactorizedTwoDotEndpointBuild, ArtifactError> {
    ...
    let selection_witness = retain_selection_witness.then_some(FactorizedTwoDotSelectionWitness {
        complete_sources,
        complete_rule,
    });
    let context = generator.context().clone();
    drop(generator);
    Ok(FactorizedTwoDotEndpointBuild {
        context,
        endpoint,
        selected_complete_source_ordinals,
        selection_witness,
...
pub(super) const fn factorized_two_dot_search_depth() -> usize {
    SEARCH_DEPTH
}
...
#[cfg(test)]
pub(super) fn derive_factorized_two_dot_candidate(
    depth: usize,
) -> Result<ParametricRule, ArtifactError> {
    let family = canonical_family()?;
    let canonicalizer = canonical_s4(&family)?;
