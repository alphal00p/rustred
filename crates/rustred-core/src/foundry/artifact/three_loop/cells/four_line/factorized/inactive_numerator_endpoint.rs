//! Exact inactive-numerator endpoint on the factorized four-line face.
//!
//! The bridge-dot bulk exposes `J(0,-1,1,1,1,1)`.  A complete depth-one
//! search selects eight of the 63 translated ordinary K6 rows.  Production
//! reprojects only those selected rows and owns exactly this endpoint's `S4`
//! orbit.  Every child is already an authenticated factorization terminal;
//! no negative-power bulk is inferred from this singleton identity.
...
pub(super) struct FactorizedFaceNumeratorSelectionWitness {
    pub(super) complete_sources: SourceViewBatch,
    pub(super) complete_rule: ParametricRule,
}

pub(super) struct FactorizedFaceNumeratorEndpointBuild {
...
pub(super) fn derive_factorized_face_numerator_endpoint_build(
    retain_selection_witness: bool,
) -> Result<FactorizedFaceNumeratorEndpointBuild, ArtifactError> {
    ...
    let selection_witness =
        retain_selection_witness.then_some(FactorizedFaceNumeratorSelectionWitness {
            complete_sources,
            complete_rule,
        });
    let context = generator.context().clone();
    drop(generator);
    Ok(FactorizedFaceNumeratorEndpointBuild {
        context,
        endpoint,
        selected_complete_source_ordinals,
        selection_witness,
...
fn derive_endpoint_rule(
    ...
    Ok(derive_sector_monotone_rule_for_target(
        ...
        &FACTORIZED_FACE_NUMERATOR_PIVOT,
...
pub(super) const fn factorized_face_numerator_search_depth() -> usize {
    SEARCH_DEPTH
}
...
#[cfg(test)]
pub(super) fn derive_factorized_face_numerator_candidate(
    depth: usize,
) -> Result<ParametricRule, ArtifactError> {
    let family = canonical_family()?;
    let canonicalizer = canonical_s4(&family)?;
