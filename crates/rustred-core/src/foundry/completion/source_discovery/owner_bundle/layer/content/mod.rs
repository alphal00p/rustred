...
fn try_build_content_id_with_overrides(
    ...
    // the paired executable owners below, while the leading regions and their
    // totality claims live only in the proof cover.
    output.count(proof.owners().len())?;
    for owner in proof.owners() {
        output.usize(owner.id().ordinal())?;
        append_u64_slice(&mut output, owner.leading().coordinates())?;
