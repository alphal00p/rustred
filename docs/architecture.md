...
| `foundry::completion` (crate-private production) | Converts rule domains into exact sector-chart carrier boxes, checks matcher/box agreement, keeps polynomial guards separate from structural coverage, and computes deterministic disjoint complements. Its physical-frame lane builds raw translated-source CSR plans, classifies every target-local column through an exact decorated-stratum role registry, and runs independent bounded probe-local obstruction campaigns. Live hits are lifted to guarded exact ordinary-source circuits with full physical replay. A scheduler-owned replay can now be rejoined after query teardown and promoted into a sealed epoch+circuit+executable-cell candidate when its guards are total on the box; intersecting or unsupported guard loci return explicit guarded-stratum obligations and confer no ownership. Proper-subsector owners come from strongly retained immutable authority; a bare RuleCell is never transitive closure authority. | Rebase independent probes onto one canonical union epoch, deduplicate before deterministic anchor selection, then add the paired semantic executable-owner compiler and transactional whole-cover swap. Continue with immutable solved-sector feedback and obstruction-block/syzygy-guided discovery; run sectors bottom-up, prove an exact all-rank owner cover, and close the remaining K6 structural complement. Machine endpoints remain explicit outer-extension obligations. |
