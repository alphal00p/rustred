...
| `foundry::completion` (test-only) | Converts rule domains into exact sector-chart carrier boxes, checks matcher/box agreement, keeps polynomial guards separate from structural coverage, and computes deterministic disjoint complements. Its physical-frame lane builds raw translated-source CSR plans, classifies every target-local column through an exact decorated-stratum role registry, and runs independent bounded probe-local obstruction campaigns. Live hits are lifted to guarded exact ordinary-source circuits with full physical replay; exhaustive guard-free misses can retain only fixed-sample dual evidence. Proper-subsector owners come from a strongly retained closed artifact or the exact K6 zero/factorization terminal authority; a bare RuleCell is never treated as transitive closure authority. The audited K6 control executes all six full-rank orbits at three primes and records the expected no-hit, obstruction-guided one-epoch frontier without promoting it. | Add maximal-stratum rebuild across request growth, certificate-/syzygy-guided residual batching, exact-circuit-to-RuleCell compilation, and immutable solved-sector feedback; then run the sectors bottom-up, replace finite-carrier diagnostics with an exact all-rank Stanley/owner cover, and close the remaining structural complement. Machine endpoints remain explicit outer-extension obligations. |
