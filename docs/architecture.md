...
| `foundry::completion` (test-only) | Converts rule domains into exact sector-chart carrier boxes, checks matcher/box agreement, keeps polynomial guards separate from structural coverage, computes deterministic disjoint box complements, and emits typed outer-extension obligations instead of treating machine-integer endpoints as mathematical infinity. Rule-cell construction now proves that all 205 current K6 guard occurrences are nonzero throughout their owning boxes. | Replace finite-carrier diagnostics with an exact all-rank outer-extension proof and close the remaining structural complement; future cells with genuinely multivariate exceptional loci still require explicit stratum owners before installation. |
