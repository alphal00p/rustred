...
| `foundry::completion` (test-only) | Converts rule domains into exact sector-chart carrier boxes, checks matcher/box agreement, keeps polynomial guards separate from structural coverage, computes deterministic disjoint box complements, and emits typed outer-extension obligations instead of treating machine-integer endpoints as mathematical infinity. | Replace finite-carrier diagnostics with an exact all-rank outer-extension proof and complete ownership of every guard-zero branch; only then may a finite complement be certified as evaluation terminals. |
