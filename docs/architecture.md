...
| `foundry::completion` (test-only) | Converts rule domains into exact sector-chart carrier boxes, checks matcher/box agreement, keeps polynomial guards separate from structural coverage, and computes deterministic disjoint complements. Its physical-frame lane now builds raw translated-source CSR plans, classifies every target-local column through an exact decorated-stratum role registry, probes those roles over Symbolica finite fields, and lifts positive modular supports back to guarded exact ordinary-source circuits with full physical replay. Only proof-backed zero, factorization, and master terminals from an immutable sealed-artifact snapshot may own a proper-subsector image; a bare RuleCell is never treated as transitive closure authority. | Run the multi-prime/held-out schedule, refine every lifted guard branch, compile admitted circuits into RuleCells, replace finite-carrier diagnostics with an exact all-rank Stanley/owner cover, and close the remaining structural complement. Machine endpoints remain explicit outer-extension obligations. |
