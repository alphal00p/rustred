...
| `src/generic_family.rs`, `automatic_isps.rs`, `base_specialization.rs`, `feynman_polynomials.rs`, `sectors.rs`, `shift_operators.rs`, `guards.rs` | move live model/normalization code to `family/` and sector/guard responsibilities to `sector/` as indicated by the DAG |
...
| `src/coordinate_equality_loci.rs`, `symbolic_sector_cases.rs`, `symmetry.rs`, `symmetry_discovery.rs`, `symbolic_symmetry_transport.rs`, `zero_sectors.rs`, `zero_sector_provider.rs`, `product_locus_boolean_cover.rs`, `residual_unit_affine_index_map.rs`, `residual_affine_{atom_rows,integer_system,integer_lattice_kernel}.rs` | move verified locus/transport/zero/factorization responsibilities to `sector/`; unsupported integer-normal-form cases remain typed boundaries rather than private CAS |
...
| existing `src/campaign/**` and `parallel_execution.rs` | move to `campaign/` and `runtime/`; keep deterministic resource/width semantics, remove facade duplication |
