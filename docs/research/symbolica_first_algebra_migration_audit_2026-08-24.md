...
The vendored Symbolica 2.2.0 public API has no functional blocker to retaining
native forward-elimination state. `SparseRowReducer` implements `Clone` and its
public `add_cols` and `add_row` preserve and extend `U`, `L`, pivots, mode, and
scratch; forward elimination itself remains serial. A RustRed stage can clone
the committed reducer, extend the trial's ordered columns, submit one row, and
...
be invoked. The `rustred-legacy-oracles` crate may be tested separately but
...
- no default-production branch depends on `rustred-legacy-oracles`.
