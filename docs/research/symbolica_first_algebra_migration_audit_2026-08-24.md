...
5. Migrate the live generated-affine exact database first through a temporary
   ...
   **Live correctness bridge complete.** The database builds the complete
   ...
   implemented and excluded from replay identity. Persistent native state and
   physical benchmark export remain separate scaling slices requiring explicit
   rollback/rebuild or poisoned-state semantics. The current full rebuild's cumulative `O(P^2)`
