...
5. Migrate the live generated-affine exact database first through a temporary
   ...
   **Live correctness bridge complete.** The database builds the complete
   ...
   default-GMP four-thread suites passed 12/12 adapter, 35/35 exact-database,
   and 2/2 direct-session tests. Migrate the older exact and parametric row
   engines afterward. Persistent native state and native-statistics retention
   are separate scaling slices requiring explicit rollback/rebuild or
   poisoned-state semantics. The current full rebuild's cumulative `O(P^2)`
   tendency and `O(K)` scratch make this a correctness result only, not a
   complete-topology, Vakint, or six-loop scalability result.
...
This sparse migration does not reorder the port's acceptance lanes. After
exceptional/subsector closure, RustRed must still compile deterministic
multi-start bundles and independently derive the complete Vakint one- through
four-loop replacement-system corpus for external-oracle comparison. Neither
lane has passed yet.
