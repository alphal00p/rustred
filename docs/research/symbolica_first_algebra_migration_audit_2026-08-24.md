...
| P0 (next) | [`generated_affine_residual_group_exact_database.rs`](../../src/generated_affine_residual_group_exact_database.rs): current-lineage incremental exact row reduction | Must move native now | Initially, a temporary per-stage `SparseRowReducer<CheckedParametricField>` rebuilt from immutable committed pivots, with hardest physical key mapped to the lowest column, one unused sentinel column, and `LuLMode::Full` | Preserve the consume-once transaction, deterministic physical ordering, guards, provenance, limits, and independent source-combination/residual replay. Symbolica must own every pivot, inverse, multiply, subtract, normalization, and dependency decision. Temporary rebuild makes panic/post-check failure transactional but is an O(P^2) correctness bridge, not a six-loop scaling result. This migration precedes the exceptional fixed-point scheduler. |
...
5. Migrate the live generated-affine exact database first through a temporary
   ...
   then migrate the older exact and parametric row engines. Keep the semantic
   controllers but move every field and row operation native; differentially
   authenticate dependency/pivot decisions, source combinations, guards, and
   exact regenerated residuals. Persistent native state is a separate scaling
   slice requiring explicit rollback/rebuild or poisoned-state semantics.
