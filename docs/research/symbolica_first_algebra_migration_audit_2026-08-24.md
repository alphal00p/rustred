...
| P0 (isolated adapter complete; live wiring next) | [`generated_affine_residual_group_exact_database.rs`](../../src/generated_affine_residual_group_exact_database.rs): current-lineage incremental exact row reduction | Native boundary exists, but the live database still uses handwritten row decisions | The checked borrowed-input adapter over temporary per-stage `SparseRowReducer<CheckedParametricField>` uses `LuLMode::Full`, an unused sentinel, shallow `Arc` native elements, a prospective output envelope, and typed failure for unused field callbacks. The next seam builds the hardest-first physical-key catalog and runs it in differential shadow against the immutable committed pivots. | Preserve the consume-once transaction, deterministic physical ordering, guards, provenance, limits, and independent source-combination/residual replay. Symbolica must own every pivot, inverse, multiply, subtract, normalization, and dependency decision before the legacy path is deleted. Temporary serial rebuild has dense `O(K)` scratch and cumulative `O(P^2)` work; it is a correctness bridge, not a six-loop scaling result. This migration precedes the exceptional fixed-point scheduler. |
...
|---|---|---|
| [`generated_affine_residual_group_exact_database.rs`](../../src/generated_affine_residual_group_exact_database.rs), [`exact_sparse_elimination.rs:315-595`](../../src/exact_sparse_elimination.rs#L315), and [`exact_sparse_elimination.rs:1399-1637`](../../src/exact_sparse_elimination.rs#L1399) | Their authenticated hardest-first integral-column/source-row controllers and provenance/replay may remain. Their private row arithmetic is not justified: column reindexing plus `SparseRowReducer` with `LuLMode::Full` supplies the required deterministic pivots and source combinations, subject to the empty/full-rank transcript caveat and transactional recovery contract above. | First rebuild a temporary reducer from immutable committed rows so failure cannot corrupt live state. Differentially compare every dependency/new-pivot decision and exact source combination, then regenerate the original relation and require a zero residual before deleting the old arithmetic. Do not call back substitution during forward closure because it mutates provenance state. |
...
5. Migrate the live generated-affine exact database first through a temporary
   ...
   **The isolated borrowed-input, `Arc`-backed checked adapter is complete; its
   licensed default-GMP focused suite passed 12/12 in parallel, while native
   authority is not yet wired into the live database.** Next build the complete
   physical-key catalog and differential
   shadow, authenticate dependency/pivot decisions, ordered `L` factors,
   normalization, source combinations, guards, and exact regenerated residuals,
   and only then delete the handwritten decision path. Migrate the older exact
   and parametric row engines afterward. Persistent native state is a separate
   scaling slice requiring explicit rollback/rebuild or poisoned-state
   semantics.
