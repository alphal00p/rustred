...
| Completed, high-loop symmetry boundary | [`symmetry_discovery.rs`](../../src/symmetry_discovery.rs) | The private Bareiss determinant and exhaustive bounded candidate enumeration were deleted. RustRed will obtain graph automorphisms, canonical maps, orbits, and group sizes directly from Symbolica's native public graph API, derive routing candidates, and feed them to the retained generic verifier; Symbolica also owns matrix algebra at that proof boundary. `feyngen` is reference evidence only. |
...
1. Migrate generic-family products and the generic Feynman/zero-sector
   polynomial boundary, because they are upstream of family construction and
   sector pruning. The private symmetry-discovery determinant has already been
   deleted with its brute-force enumerator; implement direct Symbolica graph
   candidate ingress separately.
