...
| P0 | The production-facing Vakint adapter is a two-loop special case. | It accepts canonical `I2L` and a deliberately small syntax at [`vakint_adapter.rs:1-15`](../../crates/rustred-legacy-oracles/src/vakint_adapter.rs#L1), owns `VakintTwoLoopAdapter`/`TwoLoopReductionPipeline` at [`vakint_adapter.rs:154-186`](../../crates/rustred-legacy-oracles/src/vakint_adapter.rs#L154), and reduces through that pipeline at [`vakint_adapter.rs:231-280`](../../crates/rustred-legacy-oracles/src/vakint_adapter.rs#L231). | Replace the production path with generic family/topology matching, generic tensor parsing/lowering, and application of freshly discovered authenticated rules.  Keep loop-specific adapters only as oracle fixtures. |
