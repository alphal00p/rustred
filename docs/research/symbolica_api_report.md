...
- Dense polynomial multiplication has another private thread-local `Vec<u32>` cache whose logical length is limited to `1 << 24` entries (64 MiB of initialized `u32` elements). Its retained allocation can exceed that figure through `Vec` growth/capacity, and it has no public capacity census, configurable limit, or trim operation (`poly/polynomial.rs`). Multiprecision-float constants have a separate private TLS cache. These remain opaque calibration charges, not resources RustRed should reimplement.
- `AtomView::get_byte_size` and `TermStreamer::get_byte_size` report logical payload sizes, not allocated capacity or RSS. `TermStreamer::max_mem_bytes` is a spill threshold rather than a hard process-memory cap. The sparse reducer exposes U/L/pivot vectors sufficiently for a shallow capacity census, but has no deep retained-byte or peak-scratch API; parallel back substitution additionally creates one dense `ncols` scratch buffer per Rayon fold worker and retains local outputs until fusion.
...
6. A universal Symbolica execution context that borrows one caller pool, configurable/inspectable TLS caches with trim support, and deep retained/peak-memory estimates for atoms, polynomials, sparse matrices, and `SparseRowReducer`. Until these exist, RustRed must use conservative external admission and calibrated RSS rather than inventing replacement algebra.
