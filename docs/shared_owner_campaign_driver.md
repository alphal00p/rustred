...
(H256 in the production preset). New runs use CP5 checkpoints whose manifest
binds the publication policy (`"ready"`), require unreserved-transfer
scheduling, and currently rejects physical subdivision. Select it only when
preparing a **new** campaign; an Ordered checkpoint refuses a Ready resume as
a policy change. Ordered remains the default.
